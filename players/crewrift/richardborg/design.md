# Richardborg — Design Specification

Richardborg is a [Player-SDK](../player_sdk/) agent that plays **Crewrift**, a
...
Richardborg plugs game-specific code into the Player SDK's two-loop runtime. Control
...
| ------------ | -------------------------------- | ---------------- | ---------------------------------- |
| Strategy     | `Strategy` → `ModeDirective`     | _which mode_     | role/phase rules over belief       |
| Mode         | `Mode.decide` → `Intent`         | _what to do now_ | intent selection, "done" detection |
| Action layer | `resolve_action` + `ActionState` | _how, over time_ | pathing, momentum, button timing   |
...
Richardborg supplies the six `AgentRuntime` type parameters and three functions:
...
no computer vision. The only image decodes richardborg performs are two sprite alpha
channels: the static `walkability map` and the dynamic `shadow` vision overlay (§4.4).

Richardborg writes its own websocket bridge (`coworld/policy_player.py`):
...
player/body absent from an array is _not visible_, which is not the same as _not
present_.
...
| `active_task_progress_pct`           | `progress bar N%`                                                                                                                     | **per-task** progress of _your current_ task; present only while in progress (`global:2441-2464`)                |
...
against walls (`castShadows`, `sim.nim:2974`). Richardborg decodes its alpha exactly
...
the server resends it on _any_ camera/player move (cache keyed on camera+origin,
...
- **roster** — keyed by player **color** (the one identity stable and unique
  ...
  **last-seen-alive fix** (world xy, facing, last-seen tick — written only from
  live `player <color>` sightings, so it _is_ "the last time/place I saw them
...
observation — the `task arrow` sprite is always _defined_ in init; what's gated is
whether arrow _objects_ (`7000+idx`) are emitted. Once a crewmate in `Playing` has
...
The roster/bodies aggregates answer _"what is true now"_ but flatten time. A
second, complementary layer answers _"what changed between frames"_: a bounded
...
- **Raw, not derived.** Occupancy (vent/task rects) and adjacency (kill-range) are
  _pure functions_ over the tape (`strategy.occupancy`), never materialised — so a
...
- **Carries observability.** Storing the camera + LoS mask means an absence from
  ...
  answers whether a region was _actually in line of sight_ that frame (true
...
_"what have I seen X doing"_ memory — a human's basis for suspicion, and the
...
- **Intervals from observation, with a small grace.** A predicate true while we
  watch a player extends one interval; it splits when we _see the player but the
  predicate is false_ (a real departure), or after an unobserved gap longer than
  `EVENT_MERGE_GRACE_TICKS`. A _brief_ unobserved gap (losing sight for a few
...
- **Raw observations, derived interpretations.** Only direct sightings are stored;
  compound signals are _queries_ over the log + life-status, never their own kind —
  e.g. _"orange followed yellow, who then died"_ = a `proximity` event toward
  yellow plus `roster["yellow"].life_status == "dead"`; _"red looked like a real
  crewmate"_ = total `task` dwell.

It is **neutral memory**, built for every role (an imposter benefits too); only
_acting_ on it (suspicion → Flee) is crewmate-gated. Meeting chat stays in
...
file, which is server-side data and never delivered to a player. Richardborg bakes
them.
...
- `task` → task list **in file order** — this order _is_ the `3000/7000+idx`
  stream index, so it maps a task signal to a world rect.
- `ventN` → a vent whose **group is the trailing digit** (same-group vents
  teleport together).
- named rooms → rooms.
- **emergency button** — _derived_: a 28×34 rect centered on the **bridge** room's
  center (`sim:789`).
...
- A cell is a routable **node** iff it contains a _reachable_ walkable pixel; the
...
- **Reachability** is a pixel flood from `home` (spawn) — ground truth, immune to a
  thin wall passing _through_ a cell.
...
> Building richardborg requires the game repo (or the vendored `croatoan.resources`)
> present.
...
_which intent best serves this stance now_, including detecting from belief that
...
| **Dick Mode**      | opt-in with `RICHARDBORG_DICK_MODE=1`; live crewmate, one-shot, and first kill cooldown is within the worst-case button walk plus buffer | while `Playing`, `call_meeting`; during the triggered meeting only if our button press opened it, `chat("haha, fuck you imposters")`, then skip-vote until the meeting closes                                                                                                                                                                                                                      |
...
is a lightweight coordination rule that also helps against non-richardborg teammates
...
**Composite intents** internally sequence _navigate-then-interact_, reusing one
...
**Aggressive experiment.** `RICHARDBORG_BE_DUMB=1` (or `BE_DUMB=1`) replaces the
...
**Dick Mode experiment.** `RICHARDBORG_DICK_MODE=1` (or `DICK_MODE=1`) arms a
...
`update_suspicion(belief)` runs every tick in the fast loop _after_ `update_belief`
...
- **Near-certain** (`confirmed_imposters`, persisted), from **consecutive**
  frame-to-frame transitions on the tape (§5.1): _witnessed kill_ (lone
  `KILL_RANGE_SQ` neighbour of a victim alive last frame, body now) and _witnessed
  vent_ — _emergence_ (vent + a `VENT_WALK_MARGIN` margin was in line of sight and
  clear last frame, occupied now) or _submersion_ (a player in the vent last frame
...
- **Graded functions** over the event log (§5.2): **vent dwell** (weak, ~flat past a
  pass-through), **body proximity** (log-LR _decreases_ with dwell — a skilled
  imposter flees, so brief presence is the only window on a killer; a long camp is a
  reporter), and **follow-to-death** (log-LR _increases_ with how long the shadowing
...
crew cluster while tasking): brief proximity, single-body passing, and _task dwell_
...
future evidence for the deterministic Bayesian model: _area-recency_,
_alibi clearing_, _vote-tally_ bandwagons (census-mapped `voting.dots`), and
_chat semantics_ (`chat_log`, §4.3). The meeting LLM already sees those signals
...
opt-in via `RICHARDBORG_LLM_MEETINGS=1` and `ANTHROPIC_API_KEY`; hosted Coworld
Bedrock is enabled by `USE_BEDROCK=true` (`upload-policy --use-bedrock`), and
local Bedrock can be forced with `RICHARDBORG_LLM_MEETINGS=1` plus
...
- `llm.py` owns provider-specific infra. The default direct Anthropic client uses
  ...
  `RICHARDBORG_LLM_MODEL`, or the Bedrock path with `BEDROCK_MODEL`.
...
richardborg/
  ...
  events.py          # RichardborgEventTracer: on_step_complete hook emitting domain.* events
...
emits the canonical _framework_ events automatically (`perception`,
...
Richardborg's own game-level events are emitted through the SDK's **domain-event
seam** (`EventEmitter` + `AgentRuntime(on_step_complete=…)`): `RichardborgEventTracer`
...
- _state / outcome_ (belief & action-state deltas): `phase_change`,
  `role_resolved`, `body_sighted`, `task_completed`, `kill_landed`, `vote_cast`.
- _attempt_ (keyed on the wire command's button edge): `task_started`,
  `kill_attempted`, `report_attempted`, `vent_attempted`, `chat_sent`.
- _decision audit_ (one compact record per tick): `decision_snapshot` links the
...
- _knowledge layer_ (the per-player event log §5.2 + the suspicion reasoning
  §10.1 _behind_ the actions — read off the finalized belief so `strategy/` stays
...
- _location tracking_ (§10.2): `occupancy_substrate` once the static grid/polylines
...
- _trace replay viewer_ (opt-in via `RICHARDBORG_TRACE=viewer` or `debug`):
...
`RICHARDBORG_LLM_TRACE_RAW=1` or `RICHARDBORG_TRACE=debug`.

**Viewer/debug verbosity.** `RICHARDBORG_TRACE=viewer` is opt-in and heavy: it emits
the `viewer_*` records used by [`viewer/index.html`](./viewer/index.html) to draw
agent-perspective replays over the map. `RICHARDBORG_TRACE=debug` includes those
...
| Voting policy                | vote the highest-posterior live suspect when `P(imp) ≥ VOTE_PROBABILITY` (§10.1), else **skip** — but always cast _something_ before the timer (not voting costs −10)                                                                                                                                                                                                                                                                                                                       |
| LLM meetings                 | direct Anthropic opt-in with `RICHARDBORG_LLM_MEETINGS=1` + `ANTHROPIC_API_KEY`; hosted/local Bedrock with `USE_BEDROCK=true` or `RICHARDBORG_LLM_MEETINGS=1` + `CLAUDE_CODE_USE_BEDROCK=1`; default direct model `claude-haiku-4-5-20251001`; default Bedrock model `us.anthropic.claude-haiku-4-5-20251001-v1:0`; deadline LLM prompt at <=96 ticks remaining and auto-submit at <=48 ticks remaining; chat cooldown is 100 ticks                                                         |
| Aggressive imposter selector | opt-in with `RICHARDBORG_BE_DUMB=1` or `BE_DUMB=1`; during `Playing`, imposters skip Pretend/Evade/ReportBody and always select Search unless kill-ready with a visible victim, then Hunt                                                                                                                                                                                                                                                                                                   |
| Dick Mode selector           | opt-in with `RICHARDBORG_DICK_MODE=1` or `DICK_MODE=1`; live crewmates make one emergency-button call when `ticks_until_kill_ready <= DICK_MAX_BUTTON_TRAVEL_TICKS + DICK_KILL_COOLDOWN_BUFFER_TICKS`; `DICK_MAX_BUTTON_TRAVEL_TICKS = 600`, `DICK_KILL_COOLDOWN_BUFFER_TICKS = 10`; chat `haha, fuck you imposters` only when our recorded button press opened the meeting, skip-vote, then resume; refused calls time out after `DICK_CALL_NO_MEETING_GRACE_TICKS = 48` and do not re-arm |
