...
class NavGraph:
    ...
    def nearest_reachable_node(self, x: int, y: int) -> Cell | None:
        ...
        return _spiral_nearest(
            lambda r, c: (r, c) in self.reachable, row, col, _SNAP_RADIUS_CELLS
        )
...
def _pixel_walkable(walkability: np.ndarray, x: int, y: int) -> bool:
    return (
        0 <= y < walkability.shape[0]
        and 0 <= x < walkability.shape[1]
        and bool(walkability[y, x])
    )
...
def _nearest_walkable_pixel(
    walkability: np.ndarray, x: int, y: int, max_radius: int
) -> Point | None:
...
def _flood_reachable_pixels(walkability: np.ndarray, seed: Point) -> np.ndarray:
    ...
    while queue:
        ...
        for dx, dy in _DIAGONALS:
            nx, ny = x + dx, y + dy
            if (
                not (0 <= ny < height and 0 <= nx < width)
                or not walkability[ny, nx]
                or reachable[ny, nx]
            ):
                continue
            if not (
                walkability[y, nx] or walkability[ny, x]
            ):  # both orthogonals blocked: no squeeze
                continue
            reachable[ny, nx] = True
            queue.append((nx, ny))
    return reachable
...
def _spiral_nearest(
    ok: Callable[[int, int], bool], row: int, col: int, max_radius: int
) -> Cell | None:
...
def build_nav_graph(
    ...
    *,
    ...
    if map_data is not None:
        reachable_pixels = _flood_reachable_pixels(
            walkability, (map_data.home.x, map_data.home.y)
        )
    else:
    ...
    graph = NavGraph(
        ...
        reachable=set(
            node_point
        ),  # every node sits on a reachable pixel by construction
        clearance=clearance,
    )
    if map_data is not None:
...
def _build_nodes(
    reachable_pixels: np.ndarray, clearance: np.ndarray, cell_size: int
) -> dict[Cell, Point]:
...
def _build_anchors(graph: NavGraph, map_data: MapData) -> None:
    ...
    for index, vent in enumerate(map_data.vents):
        ...
        anchor = _find_anchor(
            ...
            cy + VENT_REACH + 1,
            (cx, cy),
            predicate=lambda px, py, cx=cx, cy=cy: (
                (px - cx) ** 2 + (py - cy) ** 2 <= VENT_REACH**2
            ),
        )
        if anchor is not None:
    ...
    if unreachable:
        _log.warning(
            "richardborg nav: %d destination(s) have no reachable anchor and cannot be "
            "navigated to: %s",
            len(unreachable),
            ", ".join(unreachable),
        )
