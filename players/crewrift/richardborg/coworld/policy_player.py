"""Richardborg's Sprite-v1 websocket bridge (design §3, AGENTS.md §Transport).
...
from players.crewrift.richardborg.action import encode_chat, encode_input
from players.crewrift.richardborg.coworld.scene import SceneState
from players.crewrift.richardborg.map import walkability_matches
    ...
    try:
        async with connect(engine_ws_url, max_size=None) as websocket:
            try:
                ...
                print(
                    "game over: server closed the connection",
                    file=sys.stderr,
                    flush=True,
                )
    finally:
