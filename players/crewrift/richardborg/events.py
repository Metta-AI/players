"""Richardborg domain trace events (design §11).
...
``RichardborgEventTracer`` is wired into :func:`...build_runtime` as the runtime's
...
- **Debug only (``RICHARDBORG_TRACE=debug``):** the entire live ``P(imposter)`` vector
...
- **Trace replay viewer (``RICHARDBORG_TRACE=viewer`` or ``debug``):** browser-ready
...
from players.crewrift.richardborg.action import BTN_A, BTN_B
from players.crewrift.richardborg.perception.constants import (
    SCREEN_HEIGHT,
    SCREEN_WIDTH,
)
from players.crewrift.richardborg.strategy.opportunity import (
    has_trackable_victim,
    kill_urgency_ticks,
)
from players.crewrift.richardborg.strategy.rule_based import (
    FLEE_ENTER_SQ,
    FLEE_EXIT_SQ,
    FLEE_STALE_TICKS,
)
from players.crewrift.richardborg.strategy.suspicion import (
    VOTE_PROBABILITY,
    _prior_imposter_p,
    top_suspect,
)
...
class RichardborgEventTracer:
    """Derive richardborg domain events from each tick's :class:`StepContext`.

    Usable directly as an ``on_step_complete`` hook: ``on_step_complete=tracer``.
    """

    def __init__(
        self, *, debug: bool | None = None, viewer: bool | None = None
    ) -> None:
        ...
        self._event_counts: dict[
            str, int
        ] = {}  # color → events logged so far (emit the new tail)
        self._life: dict[
            str, str
        ] = {}  # color → last-seen life_status (alive→dead edge)
        self._confirmed: set[str] = (
            set()
        )  # last confirmed_imposters (witnessed catches)
        ...
        self._kill_ready: bool | None = (
            None  # last self_kill_ready (imposter cooldown edges)
        )
        ...
        trace_level = os.environ.get("RICHARDBORG_TRACE", "").strip().lower()
        ...
        self._viewer: bool = (
            trace_level in {"viewer", "debug"} if viewer is None else viewer
        )

    def __call__(
        self, context: StepContext[Belief, ActionState, Intent, Command]
    ) -> None:
    ...
    def _observe_completed_tasks(self, belief: Belief, emit: EventEmitter) -> None:
        for index in sorted(
            belief.completed_task_indices - self._completed_task_indices
        ):
            emit.event(
                "task_completed",
                {
                    "task_index": index,
                    "crew_tasks_remaining": belief.crew_tasks_remaining,
                },
            )
    ...
    def _observe_kill_landed(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        if (
            ...
            and belief.last_kill_tick != self._last_kill_tick
        ):
            self._last_kill_tick = belief.last_kill_tick
            emit.event(
                "kill_landed",
                {"world_x": belief.self_world_x, "world_y": belief.self_world_y},
            )
    ...
    def _observe_action(
        self, intent: Intent, command: Command, emit: EventEmitter
    ) -> None:
        kind = intent.kind
    ...
    def _observe_decision_snapshot(
        self, context: StepContext[Belief, ActionState, Intent, Command]
    ) -> None:
            ...
            if record.life_status == "dead" and self._life.get(color) != "dead":
                emit.event(
                    "player_died",
                    {
                        ...
                        "body_xy": list(record.body_xy)
                        if record.body_xy is not None
                        else None,
                    },
                )
                emit.counter(
                    "player_died", tags={"source": record.death_source or "unknown"}
                )
    ...
    def _observe_suspicion_deltas(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        for color in sorted(belief.confirmed_imposters - self._confirmed):
            emit.event(
                "imposter_confirmed",
                {"color": color, "p": round(belief.suspicion.get(color, 1.0), 4)},
            )
    ...
    def _observe_meeting_suspicion(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        ranking = [
            ...
            for color, p in sorted(
                belief.suspicion.items(), key=lambda kv: kv[1], reverse=True
            )
        ]
        emit.event(
            "suspicion_snapshot",
            {
                ...
                "believed": sorted(belief.believed_imposters),
                "would_vote": target,
                "would_vote_p": round(belief.suspicion[target], 4)
    ...
    def _observe_kill_readiness(
        self, belief: Belief, emit: EventEmitter, mode: str
    ) -> None:
    ...
    def _observe_debug_tick(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        emit.event(
            "suspicion_tick",
            {"p": {c: round(p, 4) for c, p in belief.suspicion.items()}},
        )
    ...
    def _observe_occupancy(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        for event in tracking.reacquisitions[self._occupancy_reacquisition_count :]:
            emit.event(
                "occupancy_reacquired",
                {
                    ...
                    "predicted_point": list(event.predicted_point)
                    if event.predicted_point is not None
                    else None,
                    "actual_point": list(event.actual_point),
                    "top_probability": round(event.top_probability, 4),
                    "distance_error": round(event.distance_error, 2)
                    if event.distance_error is not None
                    else None,
        ...
        emit.event(
            "occupancy_seek_target",
            {
                ...
                "point": list(snapshot.top_point)
                if snapshot.top_point is not None
                else None,
    ...
    def _observe_occupancy_debug(self, belief: Belief, emit: EventEmitter) -> None:
        ...
        top = [
            ...
            for cell_id, expected in sorted(
                snapshot.expected_by_cell.items(),
            ...
            emit.event(
                "viewer_occupancy_grid",
                {
                    ...
                    "cells": [
                        ...
                        for cell in sorted(
                            substrate.cells.values(), key=lambda item: item.index
                        )
                    ],
                },
            )
...
def _decision_snapshot_payload(
    context: StepContext[Belief, ActionState, Intent, Command],
) -> dict[str, Any]:
...
def _decision_visible_players(
    belief: Belief, visible_colors: set[str]
) -> list[dict[str, Any]]:
...
def _decision_visible_bodies(belief: Belief) -> list[dict[str, Any]]:
    return [
        ...
        for body in sorted(
            (
                belief.bodies[body_id]
                for body_id in belief.visible_body_ids
                if body_id in belief.bodies
            ),
...
def _decision_player_threat_payload(
    ...
    age_ticks = (
        None if record is None else max(0, belief.last_tick - record.last_seen_tick)
    )
    return {
        "color": color,
...
def _decision_flee_payload(
    ...
    return {
        ...
        "target_visible": target_color in visible_colors
        if target_color is not None
        else None,
        "target_last_seen_tick": target.last_seen_tick if target is not None else None,
        "target_age_ticks": (
            max(0, belief.last_tick - target.last_seen_tick)
            if target is not None
            else None
        ),
...
def _viewer_frame_payload(
    ...
    return {
        ...
        "nav": {
            ...
            "route": route,
            "teleports": {
                str(key): value for key, value in action_state.route_teleports.items()
            },
        },
        "tasks": {
        ...
        "players": [
            {
                ...
                "suspicion": round(belief.suspicion[color], 4)
                if color in belief.suspicion
                else None,
                "believed_imposter": color in belief.believed_imposters,
                "confirmed_imposter": color in belief.confirmed_imposters,
...
def _viewer_occupancy_payload(belief: Belief) -> dict[str, Any] | None:
    ...
    return {
        ...
        "teammate_cells": [
            [cell, round(expected, 4)]
            for cell, expected in sorted(
                (
                    teammate_snapshot.expected_by_cell if teammate_snapshot else {}
                ).items()
            )
            if expected > 0
        ],
