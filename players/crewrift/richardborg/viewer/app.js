...
const state = {
  ...
  overlays: Object.fromEntries(
    overlayDefs.map(([key, , enabled]) => [key, enabled]),
  ),
...
function bindControls() {
  ...
  document
    .getElementById("jumpStart")
    .addEventListener("click", () => setFrame(0));
  document
    .getElementById("stepBack")
    .addEventListener("click", () => setFrame(state.frameIndex - 1));
  document
    .getElementById("stepForward")
    .addEventListener("click", () => setFrame(state.frameIndex + 1));
  document
    .getElementById("jumpEnd")
    .addEventListener("click", () => setFrame(state.frames.length - 1));
  ...
  els.timeline.addEventListener("input", () =>
    setFrame(Number(els.timeline.value)),
  );
...
function parseTrace(text) {
  ...
  const gridEvent = events.find(
    (event) => event.name === "domain.viewer_occupancy_grid",
  );
    ...
    .map((tick) => {
      ...
      return {
        ...
        data: {
          ...
          role: "-",
          mode: {
            name: action && action.data ? action.data.mode : "-",
            params: {},
            reason: "",
          },
          intent: {
            kind: action && action.data ? action.data.intent || "-" : "-",
          },
          nav: {},
          occupancy: null,
          players: [],
...
function inferMap(frames) {
  ...
  for (const frame of frames) {
    ...
    for (const point of [
      data.nav && data.nav.target,
      data.nav && data.nav.route_goal,
    ]) {
...
function playbackLoop(now) {
  if (state.playing && state.frames.length > 1) {
    ...
    if (now - state.lastPlaybackTime >= delay) {
      const next =
        state.frameIndex +
        Math.max(1, Math.floor((now - state.lastPlaybackTime) / delay));
...
function updateInspector() {
  ...
  const tickEvents =
    frame && frame.tick !== null
      ? state.eventsByTick.get(frame.tick) || []
      : [];
  const visibleEvents = tickEvents.filter(
    (event) => !event.name.startsWith("domain.viewer_"),
  );
  ...
  els.intentValue.textContent = valueText(
    data.intent && (data.intent.kind || data.intent),
  );
  ...
  els.modeAge.textContent =
    mode.age_ticks === undefined ? "-" : `${mode.age_ticks} ticks`;
  ...
  els.navRoute.textContent = nav.route
    ? `${nav.route.length} points, cursor ${nav.route_cursor || 0}`
    : "-";
  els.occTopCell.textContent = valueText(occ && occ.top_cell);
  els.occExpected.textContent =
    occ && occ.top_expected !== undefined ? String(occ.top_expected) : "-";
  els.occTracked.textContent = occ
    ? `${occ.tracked || 0} agents, ${occ.support_cells || 0} cells`
    : "-";
...
function fitView(markFitted) {
  ...
  state.view.offsetX =
    (rect.width - bounds.width * state.view.scale) / 2 -
    bounds.x * state.view.scale;
  state.view.offsetY =
    (rect.height - bounds.height * state.view.scale) / 2 -
    bounds.y * state.view.scale;
...
function mapBounds() {
  if (state.map) {
    return {
      x: 0,
      y: 0,
      width: state.map.width || 128,
      height: state.map.height || 128,
    };
  }
  const frame = currentFrame();
  ...
  return inferred
    ? { x: 0, y: 0, width: inferred.width, height: inferred.height }
    : null;
}
...
function drawPlayers(data) {
  ...
  for (const player of players) {
    ...
    const radius =
      player.believed_imposter || player.confirmed_imposter ? 7 : 5;
    ctx.fillStyle = player.teammate
      ? "#2dd4bf"
      : player.confirmed_imposter
        ? "#ef4444"
        : player.believed_imposter
          ? "#f97316"
          : "#f4f1e8";
    ...
    if (player.suspicion !== null && player.suspicion !== undefined) {
      drawWorldLabel(
        `${player.color} ${Math.round(player.suspicion * 100)}%`,
        player.x + 8,
        player.y - 7,
        "#f4f1e8",
        "left",
...
function drawBodies(data) {
  ...
  for (const body of bodies) {
    ...
    ctx.beginPath();
    ctx.rect(
      body.x - 5 / state.view.scale,
      body.y - 3 / state.view.scale,
      10 / state.view.scale,
      6 / state.view.scale,
    );
    ctx.fill();
