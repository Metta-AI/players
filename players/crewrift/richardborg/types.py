"""Richardborg's SDK type parameters and the three pure perception functions.

Richardborg supplies the six ``AgentRuntime`` type parameters
(``Observation``/``Percept``/``Belief``/``ActionState``/``Intent``/``Command``)
plus ``perceive``/``update_belief`` here, and ``resolve_action`` in
:mod:`players.crewrift.richardborg.action`. See ``design.md`` §2.
...
from players.crewrift.richardborg.agent_tracking import AgentTrackingState
...
from players.crewrift.richardborg.nav import NavGraph, build_nav_graph
from players.crewrift.richardborg.perception.constants import PLAYER_OBJECT_BASE
from players.crewrift.richardborg.perception.entities import ResolvedScene, VotingState
from players.crewrift.richardborg.perception.resolve import resolve_scene
...
class ActionState(BaseModel):
    ...
    # Locally tracked server vote cursor. The rendered cursor can be noisy, so the
    # action layer initializes from the game cursor invariant and then follows
    # sim.nim's moveCursor rules for our own emitted inputs.
    ...
    vote_confirm_attempts: int = 0
    # If confirming the intended target does not produce a vote dot, keep cycling
    # through selectable cells and confirming until any vote is accepted.
    vote_confirm_any_target: bool = False
