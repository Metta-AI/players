...
Maintain, for every _other_ agent, a belief about **where they are now** that stays
useful after they leave view — a distribution over the ship that collapses to a point
when we see them and degrades _intelligently_ (not uniformly) when we don't. First
...
2. **Agents navigate to a destination** (usually a task; also button/corpse/player), not
   ...
   finished task, persist a direction. _These assume reasonably good opponents. Decided
   acceptable (§11-A): if opponents are instead inefficient they finish tasks slowly,
   which buys us hunting time, so the mismatch is a minor cost, not fatal._

**The reframing that makes it cheap:** the only real _latents_ are **(i) which tasks each
...
- _(Optional, non-load-bearing)_ room/region **labels** per grid cell, purely so traces
...
- **Destination mixture `π_a(t)`** over `a`'s _un-completed likely-assigned_ tasks:
...
- **Predict (closed form, pixel-level).** For each hypothesis `t`, the predicted position
  ...
  predicted pixel is _in_ a corridor when the agent is between rooms).
...
- **Re-pick (single hop, extensible).** When `t_now` exceeds a hypothesis's ETA, the agent
  would have _arrived_; for v1 that hypothesis's mass decays to **uniform-within-the-disc**
...
- **The coarse grid is also the prediction error model.** Our polylines are only an
  _approximation_ of each agent's real path (different bake / grid discretization / wall
...
1. **Static substrate + tracing.** Anchors, pairwise A* polylines, coarse grid, and a
   `domain.occupancy\_*`/predicted-vs-actual trace. **Implemented in
...
4. **Re-pick decay** + tuning (grid size, priors) against the §9 numbers.
   _(Later, gated on payoff: multi-hop re-pick, lone-target scoring, crew/voting consumer.)_
...
- **2 (target):** optimize for _any_ crew (witnessed kills unpunished by current opponents);
