...
**Status:** living document. This is the canonical, durable home for richardborg's
...
For a **crewmate** observer, for every _other_ player, the posterior probability
...
- Because a player's role is a **fixed latent variable**, evidence does not decay in
  ...
  from the _body-proximity_ function decreasing with _dwell duration_ — that's about
...
_features_ and guilt is not flat — and is sometimes **inverted**. A skilled imposter
**flees** a kill instantly; they do not loiter. So:

- A long dwell next to a body is **reporter** behaviour (innocent); a _brief_
...
- Standing on a vent is weak either way (a real venter _teleports_ — caught by the
...
| witnessed vent | tape transition (§5.1) | _emergence_ (vent + `VENT_WALK_MARGIN` in line of sight & clear last frame, occupied now) or _submersion_ (player in the vent last frame, gone while it stays in sight); LoS via the `shadow` mask (§4.4) | `WITNESSED_LOG_LR`          |

These are definitional (we _saw_ it) and not learned.
...
**provenance log** (§7). Tests assert _relational_ behaviour (evidence raises P; one
...
- **Brief proximity** to a _living_ player — crew constantly pass within kill range.
...
| the per-event log-LR functions + their constants | §3.3        | how much each graded cue moves belief, _and its shape_ vs. duration/distance. **The main thing to fit.**                                                                                                                                                        |
...
1. **Naive Bayes (conditional independence).** We treat evidence types as
   ...
   by counting each _type_ once and by conservative weights. A joint model is the
   eventual fix.
...
3. **Static prior.** We use `K / (P − 1)` and don't redistribute the imposter
   ...
   the _other_ players' calibration. A proper joint/sequential model is a refinement.
4. **Observer-relative evidence.** Suspicion is built only from what _this_ agent
...
_shape_ (this is exactly how we found body-proximity should _decrease_):
...
2. **Reconstruct observations from an observer's POV.** Evidence is what a crewmate
   _saw_, so re-run the event log + tape detectors as if richardborg were a particular
...
3. **Bin by feature and estimate the ratio per bin** (with an _opportunity_
...
7. **Update §3.3 + the provenance log (§7), then mirror into `suspicion.py`.** Re-run
   the suspicion tests; they assert _relational_ properties (evidence raises P, one
...
2. **Write its `_<cue>_log_lr(event[, belief]) -> float`** — a small closed-form
   function of the event's features (think about the _shape_: does the cue get more
