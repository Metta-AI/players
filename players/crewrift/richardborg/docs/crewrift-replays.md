...
REPLAY_DIR=/tmp/richardborg-tournament-out          # dir containing replay.json
...
1. **Crewrift never reads `COGAME_REPLAY_SERVER`.** The server's `replayServerMode`
   is set _only_ at startup from `replayLoaded` (`server.nim:664`), which is true
...
| `COGAME_SAVE_REPLAY_URI`                   | _write_ target where a live game saves its replay. Do **not** set together with load — `"Cannot save and load a replay together"` (`server.nim:637-638`).    |
...
**Implication:** during replay you will see the _normal game lifecycle_ in the logs
...
automatically. (For a _real_ tournament-style game rather than the 1-task/300-tick
certification smoke, patch a manifest copy so `certification.game_config` =
`variants[0].game_config` before `run-episode` — see
[`richardborg-tournament-ops` memory] / the steps in this repo's notes.) You can also
...
## Verifying a replay is _actually_ playing (not a live game)
...
imposters=2` and gameplay events. A _broken/live_ container stays at
