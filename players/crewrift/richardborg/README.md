...
social-deduction game (Among Us–style). Richardborg plugs Crewrift-specific
...
Richardborg plays **both roles** end-to-end. As a crewmate it does tasks, attends
...
for non-fresh visible bodies, **Hunt** (kill ready _and_ a victim visible → commit
...
reuse **Attend Meeting**. With `RICHARDBORG_LLM_MEETINGS=1` plus `ANTHROPIC_API_KEY`,
...
`RICHARDBORG_DICK_MODE=1` (or `DICK_MODE=1`), live crewmates interrupt normal tasking
...
richardborg/
  ...
  events.py          RichardborgEventTracer: on_step_complete hook → domain.* events
  ...
  scripts/play_local.sh      run richardborg against a local Crewrift server
...
```sh
uv sync --extra test
uv run pytest players/crewrift/richardborg/tests
...
```sh
players/crewrift/richardborg/scripts/play_local.sh
```

`COGAMES_ENGINE_WS_URL` defaults to `ws://localhost:2000/player?slot=0&token=`;
override it to point elsewhere.

Set `RICHARDBORG_BE_DUMB=1` (or `BE_DUMB=1`) for the aggressive imposter experiment:
...
Bedrock-capable service account. Set `RICHARDBORG_LLM_MEETINGS=1` for direct
Anthropic, or to force Bedrock locally with `CLAUDE_CODE_USE_BEDROCK=1`. Override
the default Haiku model with `RICHARDBORG_LLM_MODEL` or `BEDROCK_MODEL`.

Set `RICHARDBORG_DICK_MODE=1` (or `DICK_MODE=1`) for the crewmate emergency-button
...
Richardborg traces its reasoning to stderr as JSON lines (per-player event log,
...
§11). Set `RICHARDBORG_TRACE=viewer` for the per-tick replay view model consumed by
the browser UI, or `RICHARDBORG_TRACE=debug` for the viewer frames plus the heavier
suspicion / kill / occupancy debug dump. Set `RICHARDBORG_LLM_TRACE_RAW=1` (or
`RICHARDBORG_TRACE=debug`) to include raw LLM request/response text.

## View trace replays

Open [`viewer/index.html`](./viewer/index.html) in a browser and load a
`logs/richardborg_slot{N}_v{V}.log` file from `scripts/fetch_episodes.sh`, or any
local stderr trace captured from `scripts/play_local.sh`. Logs generated with
`RICHARDBORG_TRACE=viewer` or `RICHARDBORG_TRACE=debug` include:
...
Download the full data for the most recent episodes richardborg played in the
hosted Crewrift league (auth via `softmax login`):

```sh
players/crewrift/richardborg/scripts/fetch_episodes.sh -n 10
players/crewrift/richardborg/scripts/fetch_episodes.sh -n 5 --version 2 --out /tmp/eps
```
...
compressed `replay.json.z`, and `logs/richardborg_slot{N}_v{V}.log` — richardborg's
...
commands _would_ cover similar ground, but as of 2026-06-02 they are **broken
...
```sh
players/crewrift/richardborg/build.sh            # build + emit manifest snippet
players/crewrift/richardborg/build.sh --no-build # only render manifests
```
