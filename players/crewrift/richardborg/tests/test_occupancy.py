...
from players.crewrift.richardborg.types import PerceptionFrame


def _frame(players, camera=(0, 0)) -> PerceptionFrame:
    return PerceptionFrame(
        tick=1, camera_x=camera[0], camera_y=camera[1], players=dict(players)
    )
...
def _masked(mask: np.ndarray, camera=(0, 0)) -> PerceptionFrame:
    return PerceptionFrame(
        tick=1, camera_x=camera[0], camera_y=camera[1], visible_mask=mask
    )
