...
from players.crewrift.richardborg.map.types import MapData, MapPoint, MapRect, Vent
...
def _frame(
    tick: int, players=None, bodies=None, camera=(0, 0), mask=None
) -> PerceptionFrame:
...
def _vent_map() -> MapData:
    return MapData(
        ...
        tasks=(),
        vents=(
            Vent(x=50, y=50, w=8, h=8, group="g", group_index=1),
        ),  # rect [50,58)x[50,58)
        rooms=(),
...
def test_lone_neighbor_of_a_just_killed_victim_is_confirmed() -> None:
    prev = _frame(
        4, players={"red": (100, 100), "green": (110, 100)}
    )  # together, alive
    curr = _frame(
        5, players={"green": (110, 100)}, bodies={"red": (100, 100)}
    )  # red now a body
...
def test_kill_is_not_attributed_when_two_players_were_in_range() -> None:
    prev = _frame(
        4, players={"red": (100, 100), "green": (110, 100), "blue": (115, 100)}
    )
    curr = _frame(
        5, players={"green": (110, 100), "blue": (115, 100)}, bodies={"red": (100, 100)}
    )
    belief = _belief(prev, curr)
    update_suspicion(belief)
...
def test_kill_with_no_visible_neighbor_implicates_no_one() -> None:
    prev = _frame(
        4, players={"red": (100, 100), "green": (200, 100)}
    )  # green far (>kill range)
...
def test_non_consecutive_frames_are_not_read_as_a_kill() -> None:
    prev = _frame(
        2, players={"red": (100, 100), "green": (110, 100)}
    )  # a meeting-sized gap
...
def test_player_emerging_into_a_watched_clear_vent_is_confirmed() -> None:
    ...
    curr = _frame(
        5, players={"red": (53, 53)}, camera=(0, 0)
    )  # now inside the vent rect
...
def test_emergence_requires_the_vent_to_have_been_watched() -> None:
    ...
    update_suspicion(belief)
    assert (
        not belief.believed_imposters
    )  # we weren't watching → can't conclude emergence


def test_a_player_near_the_vent_last_frame_blocks_an_emergence_call() -> None:
    prev = _frame(
        4, players={"red": (48, 53)}, camera=(0, 0)
    )  # within the walk margin of the vent
...
def test_player_vanishing_from_a_visible_vent_is_confirmed() -> None:
    prev = _frame(
        4, players={"red": (53, 53)}, camera=(0, 0)
    )  # standing in the vent rect
...
def test_submersion_requires_the_vent_to_still_be_in_view() -> None:
    ...
    curr = _frame(
        5, players={}, camera=(400, 400)
    )  # vent off-screen now → maybe just walked off
    belief = _belief(prev, curr, map=_vent_map())
    update_suspicion(belief)
    assert not belief.believed_imposters
...
def test_a_player_standing_on_a_vent_is_not_a_venter() -> None:
    prev = _frame(4, players={"red": (53, 53)}, camera=(0, 0))
    curr = _frame(
        5, players={"red": (54, 53)}, camera=(0, 0)
    )  # still visible on the vent
    belief = _belief(prev, curr, map=_vent_map())
    update_suspicion(belief)
    assert not belief.believed_imposters
...
def test_emergence_is_suppressed_when_the_vent_is_occluded() -> None:
    ...
    prev = _frame(
        4, players={}, camera=(0, 0), mask=occluded
    )  # "clear" only because occluded
    curr = _frame(
        5,
        players={"red": (53, 53)},
        camera=(0, 0),
...
def test_submersion_is_suppressed_when_the_vent_is_occluded_now() -> None:
    ...
    assert (
        not belief.believed_imposters
    )  # player gone, but maybe they just walked behind a wall
...
def _vent_dwell(start: int = 1, dur: int = 10) -> PlayerEvent:
    return PlayerEvent(
        kind="vent", start_tick=start, end_tick=start + dur, region_index=0
    )
...
def _long_follow(start: int = 40, target: str = "yellow") -> PlayerEvent:
    return PlayerEvent(
        kind="proximity",
        start_tick=start,
        end_tick=start + FOLLOW_FULL_TICKS,
        target_color=target,
        min_dist=10,
    )
...
def _add(belief: Belief, color: str, events=()) -> None:
    belief.roster[color] = PlayerRecord(
        color=color, life_status="alive", events=list(events)
    )
...
def test_one_graded_signal_raises_the_posterior_but_does_not_flee() -> None:
    ...
    update_suspicion(belief)
    assert (
        belief.suspicion["red"] > belief.suspicion["blue"]
    )  # evidence moved the posterior up
    assert (
        "red" not in belief.believed_imposters
    )  # ...but a single cue isn't near-certain


def test_corroborating_graded_signals_cross_the_flee_bar() -> None:
    belief = _crew_belief()
    _add(belief, "red", [_vent_dwell(), _long_follow()])
    belief.roster["yellow"] = PlayerRecord(
        color="yellow", life_status="dead", death_seen_tick=40 + FOLLOW_FULL_TICKS
    )
    update_suspicion(belief)
    assert (
        belief.suspicion["red"] >= FLEE_PROBABILITY
        and "red" in belief.believed_imposters
    )
...
def test_body_proximity_is_more_suspicious_when_brief_than_when_camped() -> None:
    ...
    _add(
        belief, "green", [_near_body(dur=BODY_FADE_TICKS)]
    )  # camped until the cue fades to 0
    _add(belief, "blue")  # baseline (prior)
    update_suspicion(belief)
    assert belief.suspicion["red"] > belief.suspicion["green"]
    assert belief.suspicion["green"] == pytest.approx(
        belief.suspicion["blue"]
    )  # long camp ⇒ neutral
...
def test_pass_through_and_distant_cues_are_neutral() -> None:
    ...
    _add(
        ...
        [
            _vent_dwell(
                dur=VENT_CROSS_TICKS - 1
            ),  # duration = VENT_CROSS_TICKS ⇒ just crossing the tile
    ...
    update_suspicion(belief)
    assert belief.suspicion["red"] == pytest.approx(
        belief.suspicion["blue"]
    )  # nothing moved the prior
...
def test_following_a_victim_to_death_only_updates_when_they_died() -> None:
    ...
    assert alive.suspicion["orange"] == pytest.approx(
        alive.suspicion["yellow"]
    )  # both at the prior

    dead = _crew_belief()
    _add(dead, "orange", [_long_follow()])
    dead.roster["yellow"] = PlayerRecord(
        color="yellow", life_status="dead", death_seen_tick=40 + FOLLOW_FULL_TICKS
    )
    update_suspicion(dead)
    assert (
        dead.suspicion["orange"] > 2 / 7
    )  # following the victim to death raised it above prior
...
def test_a_confirmation_drives_the_posterior_to_near_one() -> None:
    belief = _belief(
        _frame(4, players={}), _frame(5, players={"red": (53, 53)}), map=_vent_map()
    )
