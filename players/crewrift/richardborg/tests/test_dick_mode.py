...
from players.crewrift.richardborg.modes.dick_mode import DICK_MODE_CHAT, DickMode
...
def test_dick_mode_taunts_again_for_a_new_meeting() -> None:
    ...
    assert (
        mode.decide(Belief(phase="Voting", phase_start_tick=100), ActionState()).kind
        == "chat"
    )
    assert (
        mode.decide(Belief(phase="Voting", phase_start_tick=100), ActionState()).kind
    ...
    next_meeting = mode.decide(
        Belief(phase="Voting", phase_start_tick=200), ActionState()
    )
