...
from players.crewrift.richardborg.strategy import RuleBasedStrategy
...
from players.crewrift.richardborg.types import ActionState, Belief, PlayerRecord
...
def _directive_with(
    ...
    memory = SharedMemory(
        belief=belief,
        action_state=action_state or ActionState(),
        active_directive=ModeDirective(mode="idle"),
    )
    directive = strategy.decide(BeliefSnapshot(tick=tick, memory=memory))
    return directive


def _crewmate_with_threat(
    *, tick: int, threat_x: int, threat_y: int = 100, last_seen_tick: int | None = None
) -> Belief:
    belief = Belief(
        phase="Playing",
        self_role="crewmate",
...
def test_body_in_view_selects_report_body() -> None:
    from players.crewrift.richardborg.types import BodyEntry
...
def test_dick_mode_triggers_once_before_kill_cooldown_ready(monkeypatch) -> None:
    monkeypatch.setenv("RICHARDBORG_DICK_MODE", "1")
    strategy = RuleBasedStrategy()
    ...
    assert (
        _select_with(strategy, belief, tick=belief.last_tick, action_state=action_state)
        == "normal"
    )
...
def test_dick_mode_button_refusal_timeout_does_not_retry(monkeypatch) -> None:
    ...
    belief.last_tick = (
        action_state.last_call_meeting_attempt_tick + DICK_CALL_NO_MEETING_GRACE_TICKS
    )
    assert (
        _select_with(strategy, belief, tick=belief.last_tick, action_state=action_state)
        == "normal"
...
def test_flee_stays_active_until_threat_is_clearly_clear() -> None:
    ...
    assert (
        _select_with(strategy, _crewmate_with_threat(tick=1, threat_x=150), tick=1)
        == "flee"
    )
    ...
    assert (
        _select_with(strategy, _crewmate_with_threat(tick=2, threat_x=175), tick=2)
        == "flee"
    )
    assert (
        _select_with(strategy, _crewmate_with_threat(tick=3, threat_x=205), tick=3)
        == "normal"
    )
...
def test_flee_exits_when_last_known_threat_position_is_stale() -> None:
    ...
    assert (
        _select_with(strategy, _crewmate_with_threat(tick=10, threat_x=150), tick=10)
        == "flee"
    )
...
def _imposter_with_visible_target(**kwargs) -> Belief:
    from players.crewrift.richardborg.types import PlayerRecord
...
def test_imposter_pretends_by_default() -> None:
    ...
    assert (
        _select(Belief(phase="Playing", self_role="imposter", last_tick=10))
        == "pretend"
    )
...
def test_imposter_evades_before_reporting_a_fresh_kill_body() -> None:
    ...
    belief = _imposter_with_visible_target(
        self_kill_ready=True, last_kill_tick=9, visible_body_ids={2003}
    )
    belief.bodies[2003] = BodyEntry(
        object_id=2003, color="green", world_x=60, world_y=60, first_seen_tick=10
    )
...
def test_imposter_can_report_a_non_fresh_visible_body() -> None:
    ...
    belief = _imposter_with_visible_target(
        self_kill_ready=True, last_kill_tick=1, visible_body_ids={2003}
    )
...
def test_be_dumb_imposter_searches_instead_of_pretending(monkeypatch) -> None:
    monkeypatch.setenv("RICHARDBORG_BE_DUMB", "1")

    belief = Belief(
        phase="Playing", self_role="imposter", self_kill_ready=False, last_tick=10
    )
    assert _select(belief) == "search"


def test_be_dumb_imposter_hunts_when_kill_ready_with_visible_victim(
    monkeypatch,
) -> None:
    monkeypatch.setenv("RICHARDBORG_BE_DUMB", "1")
...
def test_be_dumb_imposter_skips_evade_and_report_body(monkeypatch) -> None:
    ...
    fresh_kill = _imposter_with_visible_target(
        self_kill_ready=True, last_kill_tick=9, visible_body_ids={2003}
    )
    fresh_kill.bodies[2003] = BodyEntry(
        object_id=2003, color="green", world_x=60, world_y=60, first_seen_tick=10
    )
    ...
    body_only = Belief(
        ...
        last_tick=10,
        visible_body_ids={2003},
    )
    body_only.bodies[2003] = BodyEntry(
        object_id=2003, color="green", world_x=60, world_y=60, first_seen_tick=10
    )
