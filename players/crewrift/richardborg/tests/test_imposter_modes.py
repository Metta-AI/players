...
def test_hunt_strikes_a_victim_in_range_and_unwitnessed() -> None:
    belief = Belief(
        self_world_x=100, self_world_y=100, last_tick=5, self_kill_ready=True
    )
...
def test_hunt_shadows_in_range_until_the_cooldown_clears() -> None:
    ...
    belief = Belief(
        self_world_x=100, self_world_y=100, last_tick=5, self_kill_ready=False
    )
...
def test_hunt_stalks_a_distant_victim() -> None:
    ...
    intent = HuntMode().decide(belief, ActionState())
    assert (
        intent.kind == "navigate_to" and intent.point[0] > 100
    )  # heading toward the victim


def test_hunt_ignores_a_recently_seen_victim() -> None:
    belief = Belief(self_world_x=100, self_world_y=100, last_tick=5)
    _visible(
        belief, 1004, (120, 100), tick=1
    )  # Search may track this; Hunt requires visibility
...
def test_hunt_skips_teammates() -> None:
    ...
    _visible(
        belief, 1007, (108, 100), color="green"
    )  # an in-range crewmate is killable
    intent = HuntMode().decide(belief, ActionState())
    assert intent.kind == "kill" and intent.target_color == "green"
...
def test_hunt_prefers_reachable_victim() -> None:
    ...
    mask[:, 56:64] = (
        False  # wall splits the map; right side is unreachable from the left
    )
...
def _shadow_map() -> MapData:
    ...
    return MapData(
        ...
        tasks=(
            ...
            TaskStation(
                name="b", x=150, y=40, w=20, h=20
            ),  # in room B, center (160, 50)
        ),
        vents=(),
        rooms=(
        ...
        button=MapRect(x=0, y=100, w=10, h=10),
...
def _belief(map_data: MapData, nav, self_xy: tuple[int, int], tick: int) -> Belief:
    return Belief(
        map=map_data,
        nav=nav,
        self_world_x=self_xy[0],
        self_world_y=self_xy[1],
        last_tick=tick,
    )
...
def test_pretend_fakes_a_task_at_the_occupancy_selected_station() -> None:
    ...
    assert (
        70 <= moving.point[0] < 90 and 40 <= moving.point[1] < 60
    )  # room A's station rect
