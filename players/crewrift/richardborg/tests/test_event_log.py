...
from players.crewrift.richardborg.strategy.event_log import (
    EVENT_MERGE_GRACE_TICKS,
    update_event_log,
)
from players.crewrift.richardborg.types import Belief, BodyEntry, PlayerRecord
...
def _map() -> MapData:
    return MapData(
        ...
        vents=(
            Vent(x=200, y=200, w=8, h=8, group="g", group_index=1),
        ),  # rect [200,208)
...
def _see(belief: Belief, color: str, xy: tuple[int, int], tick: int) -> None:
    ...
    rec.world_x, rec.world_y, rec.last_seen_tick, rec.life_status = (
        xy[0],
        xy[1],
        tick,
        "alive",
...
def test_task_dwell_accumulates_one_interval_while_visible() -> None:
    ...
    assert events[0].duration_ticks == 3 and (
        events[0].start_tick,
        events[0].end_tick,
    ) == (1, 3)
...
def test_room_and_vent_dwell_are_logged() -> None:
    ...
    assert any(
        e.kind == "vent" and e.region_index == 0 for e in belief.roster["blue"].events
    )
    assert any(
        e.kind == "room" and e.region_index == 0 for e in belief.roster["green"].events
    )
...
def test_near_body_logs_target_color_and_closest_approach() -> None:
    ...
    belief.bodies[2001] = BodyEntry(
        object_id=2001, color="yellow", world_x=300, world_y=300, first_seen_tick=1
    )
    ...
    assert len(near) == 1
    assert (
        near[0].target_color == "yellow" and near[0].min_dist == 10
    )  # closest approach kept
...
def test_proximity_is_logged_for_both_players() -> None:
    ...
    for tick in (1, 2):
        ...
        belief.roster["orange"].last_seen_tick, belief.roster["orange"].life_status = (
            tick,
            "alive",
        )
        belief.roster["yellow"].world_x, belief.roster["yellow"].world_y = 310, 300
        belief.roster["yellow"].last_seen_tick, belief.roster["yellow"].life_status = (
            tick,
            "alive",
        )
        belief.last_tick = tick
...
def test_following_then_death_is_a_query_over_the_log() -> None:
    ...
    belief = Belief(map=_map())
    belief.roster["orange"] = PlayerRecord(
        color="orange", world_x=300, world_y=300, last_seen_tick=1, life_status="alive"
    )
    belief.roster["yellow"] = PlayerRecord(
        color="yellow", world_x=308, world_y=300, last_seen_tick=1, life_status="alive"
    )
    belief.last_tick = 1
    update_event_log(belief)
    ...
    followed = [
        e
        for e in belief.roster["orange"].events
...
def test_dead_players_are_not_logged() -> None:
    ...
    belief.roster["red"] = PlayerRecord(
        color="red", world_x=110, world_y=110, last_seen_tick=5, life_status="dead"
    )
    belief.last_tick = 5
    update_event_log(belief)
