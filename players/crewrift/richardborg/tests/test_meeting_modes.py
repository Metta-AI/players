...
def test_attend_meeting_skips_only_when_no_legal_vote_target_exists() -> None:
    mode = AttendMeetingMode()
    belief = Belief(phase="Voting")
...
def test_attend_meeting_votes_a_legal_target_instead_of_skipping() -> None:
    mode = AttendMeetingMode()
...
def test_attend_meeting_llm_skip_submission_uses_a_legal_vote_target() -> None:
    client = _FakeMeetingClient(
        [MeetingDecision(action="submit_vote", vote_target=VOTE_SKIP)]
    )
