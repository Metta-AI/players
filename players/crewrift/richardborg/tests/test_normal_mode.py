...
from players.crewrift.richardborg.modes import NormalMode
from players.crewrift.richardborg.nav import build_nav_graph
from players.crewrift.richardborg.types import ActionState, Belief
...
def test_bubble_flicker_at_low_progress_does_not_complete() -> None:
    ...
    assert (
        intent.kind == "complete_task" and intent.task_index == 0
    )  # still holding task 0
...
def test_returns_to_the_start_room_when_all_tasks_are_done() -> None:
    ...
    intent = NormalMode().decide(belief, ActionState())
    assert intent.kind == "navigate_to" and intent.point == (
        0,
        0,
    )  # back to spawn, not idle
...
def test_sweeps_baked_tasks_when_no_signals_arrive() -> None:
    ...
    belief = Belief(
        map=_map_with_tasks(), self_world_x=0, self_world_y=0, crew_tasks_remaining=5
    )
    intent = NormalMode().decide(belief, ActionState())
...
def test_no_sweep_once_crew_tasks_are_done() -> None:
    ...
    belief = Belief(
        map=_map_with_tasks(), self_world_x=0, self_world_y=0, crew_tasks_remaining=0
    )
    intent = NormalMode().decide(belief, ActionState())
    assert intent.kind == "navigate_to" and intent.point == (
        0,
        0,
    )  # home, not a station sweep
