...
from players.crewrift.richardborg.types import Belief, PlayerRecord
...
def test_ticks_until_kill_ready() -> None:
    ...
    assert (
        ticks_until_kill_ready(Belief(self_kill_ready=False))
        == DEFAULT_KILL_COOLDOWN_TICKS
    )
...
def _crew(
    belief: Belief, object_id: int, xy: tuple[int, int], color: str, tick: int
) -> None:
...
def test_kill_urgency_is_zero_until_kill_ready() -> None:
    ...
    assert (
        kill_urgency_ticks(Belief(last_tick=100, self_kill_ready=True)) == 0
    )  # since-tick unknown
    assert (
        kill_urgency_ticks(
            Belief(last_tick=100, self_kill_ready=True, kill_ready_since_tick=70)
        )
        == 30
    )
...
def test_trackable_when_a_crewmate_was_seen_recently() -> None:
    ...
    _crew(
        belief, 1, (50, 50), "green", 200 - (TRACK_WINDOW_TICKS - 1)
    )  # within the window
...
def test_select_victim_prefers_unclaimed_target_when_teammate_is_closer() -> None:
    belief = Belief(
        self_world_x=0, self_world_y=0, last_tick=5, teammate_colors={"pink"}
    )
...
def test_select_victim_still_takes_claimed_target_if_it_is_the_only_option() -> None:
    ...
    _crew(belief, 1, (100, 0), "green", 5)
    _crew(
        belief, 2, (96, 0), "pink", 5
    )  # teammate is closer, but there is no other victim
...
def test_full_urgency_strikes_through_a_witness() -> None:
    belief = Belief(
        self_world_x=0,
        self_world_y=0,
        last_tick=URGENCY_FULL_TICKS,
        self_kill_ready=True,
        kill_ready_since_tick=0,
    )
    _crew(belief, 1, (50, 50), "green", URGENCY_FULL_TICKS)
    _crew(
        belief, 2, (60, 50), "blue", URGENCY_FULL_TICKS
    )  # witness ignored at full urgency
    assert unwitnessed(belief, belief.roster["green"])
