...
from players.crewrift.richardborg.action import INPUT_HEADER, encode_chat
from players.crewrift.richardborg.coworld.policy_player import run_bridge
from players.crewrift.richardborg.tests import sprite_wire as w
from players.crewrift.richardborg.types import Command
...
async def test_bridge_sends_chat_packet() -> None:
    ...
    class ChattyRuntime:
        ...
        def step(self, _observation) -> Command:
            self.steps += 1
            return (
                Command(held_mask=0, chat="gg")
                if self.steps == 1
                else Command(held_mask=0)
            )
    ...
    async with serve(handler, "localhost", 0) as server:
        ...
        url = f"ws://localhost:{port}/player?slot=0&token="
        await asyncio.wait_for(
            run_bridge(url, build=lambda **_: ChattyRuntime()), timeout=5.0
        )
