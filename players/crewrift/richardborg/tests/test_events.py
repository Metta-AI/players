"""Unit tests for the richardborg domain-event tracer (events.py).
...
from players.crewrift.richardborg.action import BTN_A, BTN_B, BTN_LEFT
from players.crewrift.richardborg.events import RichardborgEventTracer
from players.crewrift.richardborg.strategy.suspicion import VOTE_PROBABILITY
...
class _Harness:
    ...
    def __init__(self, *, debug: bool | None = None, viewer: bool = False) -> None:
        ...
        # Pin debug explicitly (default off) so an ambient RICHARDBORG_TRACE=debug in the
        # test environment can't perturb the lean-mode assertions.
        self.tracer = RichardborgEventTracer(debug=bool(debug), viewer=viewer)
...
def test_decision_snapshot_includes_visibility_threat_task_and_command_geometry() -> (
    None
):
...
def test_player_event_emitted_for_each_newly_opened_interval() -> None:
    from players.crewrift.richardborg.types import PlayerEvent, PlayerRecord
...
def test_occupancy_substrate_and_seek_target_events_are_lean() -> None:
    ...
    from players.crewrift.richardborg.agent_tracking import (
        OccupancySnapshot,
        update_agent_tracking,
    )
    from players.crewrift.richardborg.map.types import MapData, MapPoint, MapRect
...
def test_occupancy_reacquisition_events_are_emitted_once() -> None:
    from players.crewrift.richardborg.agent_tracking import ReacquisitionEvent
...
def test_env_flag_enables_debug_dump(monkeypatch) -> None:
    monkeypatch.setenv("RICHARDBORG_TRACE", "debug")
    tracer = RichardborgEventTracer()
    assert tracer._debug is True
    assert tracer._viewer is True
    monkeypatch.setenv("RICHARDBORG_TRACE", "")
    assert RichardborgEventTracer()._debug is False
    monkeypatch.setenv("RICHARDBORG_TRACE", "viewer")
    tracer = RichardborgEventTracer()
    assert tracer._viewer is True
...
def test_build_runtime_wires_the_tracer_as_on_step_complete() -> None:
    ...
    assert isinstance(runtime.on_step_complete, RichardborgEventTracer)
...
def test_domain_event_flows_through_a_real_runtime_step() -> None:
    ...
    from players.crewrift.richardborg import build_runtime
    from players.crewrift.richardborg.coworld.scene import SceneState
    from players.crewrift.richardborg.tests import sprite_wire as w
    from players.crewrift.richardborg.types import Observation
