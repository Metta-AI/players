...
from players.crewrift.richardborg.perception.entities import ResolvedScene
from players.crewrift.richardborg.types import Belief, Percept, update_belief
...
def test_just_killed_recorded_on_kill_ready_to_cooldown_edge() -> None:
    ...
    _fold(
        belief, 6, self_role="imposter", self_kill_ready=False, crew_tasks_remaining=3
    )
    assert belief.last_kill_tick == 6
    # Staying on cooldown does not re-record.
    _fold(
        belief, 7, self_role="imposter", self_kill_ready=False, crew_tasks_remaining=3
    )
    assert belief.last_kill_tick == 6
...
def test_kill_ready_since_tick_tracks_the_cooldown_to_ready_edge() -> None:
    ...
    _fold(
        belief, 5, self_role="imposter", self_kill_ready=False, crew_tasks_remaining=3
    )
    ...
    _fold(
        belief, 10, self_role="imposter", self_kill_ready=False, crew_tasks_remaining=3
    )
    assert belief.kill_ready_since_tick is None
...
def test_roster_accumulates_a_sighting_trail() -> None:
    from players.crewrift.richardborg.perception.entities import VisiblePlayer

    belief = Belief()
    _fold(
        belief,
        1,
        crew_tasks_remaining=5,
        visible_players=(
            VisiblePlayer(
                object_id=1001, color="green", facing="left", world_x=10, world_y=10
            ),
        ),
    )
    _fold(
        ...
        visible_players=(
            VisiblePlayer(
                object_id=1001, color="green", facing="right", world_x=14, world_y=12
            ),
        ),
    )
    ...
    assert (entry.world_x, entry.world_y, entry.last_seen_tick, entry.facing) == (
        14,
        12,
        2,
        "right",
...
def test_roster_history_is_bounded() -> None:
    from players.crewrift.richardborg.types import ROSTER_HISTORY_MAX, PlayerRecord

    entry = PlayerRecord(
        object_id=1, color="red", facing="left", world_x=0, world_y=0, last_seen_tick=0
    )
...
def test_no_false_kill_after_a_meeting() -> None:
    ...
    belief = Belief()
    _fold(
        belief, 1, self_role="imposter", self_kill_ready=True, crew_tasks_remaining=3
    )  # Playing, ready
    ...
    _fold(
        belief, 3, self_role="imposter", self_kill_ready=False, crew_tasks_remaining=3
    )
    assert belief.phase == "Playing"
    assert belief.last_kill_tick is None
...
def test_tape_records_camera_ready_frames_with_viewport_and_entities() -> None:
    ...
    resolved = ResolvedScene(
        ...
        visible_players=(
            VisiblePlayer(
                object_id=1001, color="green", facing="left", world_x=50, world_y=80
            ),
        ),
        visible_bodies=(
            VisibleBody(object_id=2002, color="red", world_x=44, world_y=72),
        ),
    )
...
def test_tape_frame_carries_the_visibility_mask() -> None:
    ...
    update_belief(
        belief,
        Percept(tick=2, messages_applied=2, resolved=resolved, visible_mask=mask),
    )
...
def test_tape_is_bounded() -> None:
    from players.crewrift.richardborg.types import RECENT_FRAMES_MAX
...
def test_body_sighting_connects_last_seen_alive_to_the_death() -> None:
    ...
    _fold(
        ...
        visible_players=(
            VisiblePlayer(
                object_id=1003, color="red", facing="left", world_x=10, world_y=10
            ),
        ),
    )
    ...
    _fold(
        ...
        9,
        visible_bodies=(
            VisibleBody(object_id=2003, color="red", world_x=300, world_y=80),
        ),
    )
...
def test_meeting_clears_in_world_bodies_but_keeps_the_death() -> None:
    ...
    _fold(
        ...
        1,
        visible_bodies=(
            VisibleBody(object_id=2003, color="red", world_x=10, world_y=10),
        ),
    )
    assert (
        belief.bodies
        and "red" in belief.roster
        and belief.roster["red"].life_status == "dead"
    )
...
def test_census_records_alive_and_dead_players_by_color() -> None:
    from players.crewrift.richardborg.perception.entities import CensusEntry
    ...
    _fold(
        ...
        4,
        census=(
            CensusEntry(color="blue", alive=True),
            CensusEntry(color="green", alive=False),
        ),
    )
...
def test_chat_log_accumulates_dedups_and_resets_each_meeting() -> None:
    from players.crewrift.richardborg.perception.entities import ChatLine, VotingState
    ...
    _fold(
        ...
        1,
        voting=voting,
        chat_lines=(ChatLine(speaker_color="red", text="i was in nav"),),
    )
    _fold(
        belief,
        2,
    ...
    assert [(e.speaker_color, e.text) for e in belief.chat_log] == [
        ("red", "i was in nav"),
        ("blue", "sus"),
    ]
    ...
    _fold(
        ...
        11,
        voting=voting,
        chat_lines=(ChatLine(speaker_color="green", text="fresh"),),
    )
