...
from players.crewrift.richardborg.nav import (
    CLEARANCE_RADIUS,
    _segment_clear,
    build_nav_graph,
    plan_route,
...
def test_route_keeps_clearance_around_a_wall() -> None:
    ...
    for x, y in route:
        assert bool(graph.clearance[y, x]), (
            f"waypoint ({x},{y}) hugs a wall (no clearance)"
        )
...
def _map(
    tasks=(), vents=(), button=MapRect(x=0, y=0, w=4, h=4), home=MapPoint(x=4, y=4)
) -> MapData:
    return MapData(
        width=48,
        height=24,
...
def test_vent_edges_connect_same_group_reachable_vents() -> None:
    ...
    pairs = {
        (e.from_vent, e.to_vent) for edges in graph.vent_edges.values() for e in edges
    }
...
def _detour_map_with_linking_vents() -> tuple[np.ndarray, MapData]:
    ...
    vents = [
        ...
        ),  # center (90, 60), left of wall
        Vent(
            x=106, y=56, w=8, h=8, group="g", group_index=2
        ),  # center (110, 60), right of wall
    ]
    map_data = MapData(
        width=200,
        height=120,
