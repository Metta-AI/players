...
from players.crewrift.richardborg.action import (
    BTN_A,
    BTN_B,
    BTN_DOWN,
    BTN_LEFT,
...
from players.crewrift.richardborg.map.types import (
    MapData,
    MapPoint,
    MapRect,
    TaskStation,
...
from players.crewrift.richardborg.nav import build_nav_graph
from players.crewrift.richardborg.perception.entities import (
    SKIP_VOTE_TARGET,
    VoteCandidate,
    VoteDot,
    VotingState,
...
def _detour_map_with_linking_vents() -> MapData:
    return MapData(
        ...
        vents=(
            Vent(
                x=86, y=56, w=8, h=8, group="g", group_index=1
            ),  # center (90, 60), left
            Vent(
                x=106, y=56, w=8, h=8, group="g", group_index=2
            ),  # center (110, 60), right
        ),
        rooms=(),
        button=MapRect(x=0, y=0, w=4, h=4),
        home=MapPoint(x=10, y=10),
    )


def test_escape_presses_b_to_vent_when_a_teleport_leg_is_next() -> None:
    mask: npt.NDArray[np.bool_] = np.ones((120, 200), dtype=bool)
    ...
    command = resolve_action(
        Intent(kind="escape", point=(190, 110)), belief, action_state
    )
...
def test_periodic_replan_reroots_the_route_on_interval() -> None:
    from players.crewrift.richardborg.action import REPLAN_INTERVAL
...
def test_escape_resumes_walking_after_the_teleport() -> None:
    ...
    belief = Belief(
        map=_detour_map_with_linking_vents(), self_world_x=110, self_world_y=60
    )
...
def test_escape_walks_before_reaching_the_vent() -> None:
    ...
    command = resolve_action(
        Intent(kind="escape", point=(190, 110)), belief, ActionState()
    )
...
def test_navigate_presses_dpad_toward_target() -> None:
    ...
    command = resolve_action(
        Intent(kind="navigate_to", point=(100, 0)), belief, ActionState()
    )
    assert command.held_mask == BTN_RIGHT

    belief = Belief(self_world_x=0, self_world_y=0)
    command = resolve_action(
        Intent(kind="navigate_to", point=(0, 100)), belief, ActionState()
    )
...
def test_navigate_releases_within_arrive_deadband() -> None:
    belief = Belief(self_world_x=0, self_world_y=0)
    command = resolve_action(
        Intent(kind="navigate_to", point=(3, 0)), belief, ActionState()
    )
    assert command.held_mask == 0
...
def test_navigate_holds_still_when_nav_route_unreachable() -> None:
    ...
    mask: npt.NDArray[np.bool_] = np.ones((24, 48), dtype=bool)
    mask[:, 24:32] = False
    belief = Belief(self_world_x=8, self_world_y=12)  # left of the wall
    belief.nav = build_nav_graph(mask, cell_size=8)
    command = resolve_action(
        Intent(kind="navigate_to", point=(40, 12)), belief, ActionState()
    )
...
def test_complete_task_holds_a_inside_rect_and_navigates_outside() -> None:
    ...
    command = resolve_action(
        Intent(kind="complete_task", task_index=0), belief_inside, ActionState()
    )
    ...
    command = resolve_action(
        Intent(kind="complete_task", task_index=0), belief_outside, ActionState()
    )
...
def test_report_in_range_edge_presses_a_refiring_requires_release() -> None:
    ...
    assert (
        resolve_action(intent, belief, action_state).held_mask == BTN_A
    )  # fresh press
    assert (
        resolve_action(intent, belief, action_state).held_mask == 0
    )  # release to reset edge
...
def test_report_out_of_range_navigates_to_body() -> None:
    ...
    command = resolve_action(
        Intent(kind="report", target_id=2003), belief, ActionState()
    )
...
def test_call_meeting_navigates_then_edge_presses_a_on_button() -> None:
    ...
    on_button = Belief(
        map=_one_task_map(), self_world_x=1, self_world_y=1, last_tick=42
    )
    action_state = ActionState()
    ...
    assert (
        resolve_action(intent, on_button, action_state).held_mask == 0
    )  # release to reset edge


def test_vote_skip_steps_to_skip_then_confirms_once() -> None:
    belief = Belief()
    candidates = (VoteCandidate(slot=0, color="red", alive=True),)
    belief.voting = VotingState(
        cursor_present=True, self_marker_color="red", candidates=candidates
    )  # not skip yet
    action_state = ActionState()
...
def test_targeted_vote_cycles_after_failed_confirm_pulses() -> None:
    belief = Belief()
    belief.voting = _vote_grid()
    action_state = ActionState()
    intent = Intent(kind="vote", target_color="blue")
    ...
    assert resolve_action(intent, belief, action_state).held_mask == BTN_RIGHT
    assert action_state.vote_confirm_any_target


def test_targeted_vote_initializes_from_first_live_candidate_when_cursor_is_noisy() -> (
    None
):
    belief = Belief()
    candidates = (
        VoteCandidate(slot=0, color="red", alive=True),
        VoteCandidate(slot=1, color="orange", alive=False),
        VoteCandidate(slot=2, color="yellow", alive=True),
        VoteCandidate(slot=3, color="light blue", alive=True),
        VoteCandidate(slot=4, color="pink", alive=True),
        VoteCandidate(slot=5, color="lime", alive=False),
        VoteCandidate(slot=6, color="blue", alive=True),
        VoteCandidate(slot=7, color="pale blue", alive=True),
    )
    belief.voting = VotingState(
        cursor_present=True,
        self_marker_color="red",
        cursor_slot=4,  # noisy render read; the server starts on first live slot 0
...
def test_targeted_vote_confirms_immediately_when_already_on_target() -> None:
    ...
    command = resolve_action(
        Intent(kind="vote", target_color="red"), belief, ActionState()
    )
...
def test_unresolvable_target_falls_back_to_skip() -> None:
    ...
    belief.voting = VotingState(
        skip_cursor_present=True,
        candidates=(VoteCandidate(slot=0, color="red", alive=True),),
    )
    command = resolve_action(
        Intent(kind="vote", target_color="purple"), belief, ActionState()
    )
...
def test_distinct_chat_intents_can_emit_after_an_intent_change() -> None:
    action_state = ActionState()
    assert (
        resolve_action(Intent(kind="chat", text="first"), Belief(), action_state).chat
        == "first"
    )
    assert resolve_action(Intent(kind="idle"), Belief(), action_state).chat is None
    assert (
        resolve_action(Intent(kind="chat", text="second"), Belief(), action_state).chat
        == "second"
    )
...
def test_flee_moves_away_from_threat() -> None:
    ...
    command = resolve_action(
        Intent(kind="flee_from", target_color="red"), belief, ActionState()
    )
...
def test_kill_navigates_then_edge_presses_a_in_range() -> None:
    ...
    assert (
        resolve_action(intent, on_target, action_state).held_mask == BTN_A
    )  # fresh press
    assert (
        resolve_action(intent, on_target, action_state).held_mask == 0
    )  # release (edge)

    far = _belief_with_target((300, 300), (50, 50))
    assert (
        resolve_action(
            Intent(kind="kill", target_color="red"), far, ActionState()
        ).held_mask
        == BTN_UP | BTN_LEFT
    )
...
def _belief_with_vent(self_xy: tuple[int, int], vent_xy: tuple[int, int]) -> Belief:
    vent = Vent(
        x=vent_xy[0] - 7, y=vent_xy[1] - 7, w=14, h=14, group="1", group_index=1
    )  # center vent_xy
    map_data = MapData(
        width=1235,
        height=659,
...
def test_vent_navigates_then_holds_b_level_in_range() -> None:
    ...
    assert (
        resolve_action(Intent(kind="vent"), far, ActionState()).held_mask
        == BTN_UP | BTN_LEFT
    )
