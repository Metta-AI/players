...
from players.crewrift.richardborg.modes import PretendMode
...
def _belief() -> Belief:
    map_data = _map()
    nav = build_nav_graph(
        np.ones((map_data.height, map_data.width), dtype=bool), map_data=map_data
    )
    return Belief(
        map=map_data, nav=nav, self_role="imposter", self_world_x=8, self_world_y=8
    )
...
def test_static_substrate_builds_anchors_polylines_and_grid() -> None:
    ...
    assert [anchor.name for anchor in substrate.anchors] == [
        "home",
        "button",
        "task:0",
        "task:1",
    ...
    assert (
        substrate.polylines[("home", "task:1")].point_at(0)
        == substrate.anchors[0].point
    )
...
def test_tracker_collapses_visible_agents_then_sweeps_visible_empty_cells() -> None:
    ...
    belief.roster["green"] = PlayerRecord(
        color="green", world_x=16, world_y=16, last_seen_tick=10, life_status="alive"
    )
    ...
    belief.recent_frames.append(
        PerceptionFrame(
            tick=11, camera_x=0, camera_y=0, visible_mask=np.ones((64, 64), dtype=bool)
        )
    )
    update_agent_tracking(belief)
    ...
    assert all(
        substrate.cells[cell_id].center[0] >= 64 for cell_id in estimate.mass_by_cell
    )
...
def test_tracker_records_predicted_vs_actual_reacquisition() -> None:
    ...
    belief.recent_frames.append(
        PerceptionFrame(tick=12, camera_x=0, camera_y=0, players={"green": (80, 16)})
    )
    update_agent_tracking(belief)
...
def test_best_pretend_room_target_penalizes_teammate_pressure() -> None:
    ...
    right_cell = next(
        cell for cell in substrate.cells.values() if cell.label == "Right"
    )
    belief.agent_tracking.snapshot = OccupancySnapshot(
        tick=1,
...
def test_pretend_commits_to_an_occupancy_room_until_mode_preemption() -> None:
    ...
    assert (
        still_committed.kind == "navigate_to" and still_committed.point == first.point
    )
