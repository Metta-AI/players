...
from players.crewrift.richardborg.map.parser import ResourceRect, load_resource_rects
...
def _is_room(name: str) -> bool:
    ...
    return (
        bool(key)
        and key not in ("vents", "tasks", "rooms")
        and not key.startswith("vent")
        and key != "task"
    )
...
def bake_map(
    ...
    tasks = tuple(
        TaskStation(
            name=_task_name(rooms, rect, i), x=rect.x, y=rect.y, w=rect.w, h=rect.h
        )
        for i, rect in enumerate(task_rects)
    )

    home = next(
        (room.center for room in rooms if _name_key(room.name) == "bridge"), None
    )
    if home is None:
...
def load_croatoan_map() -> MapData:
    ...
    text = (
        resources.files("players.crewrift.richardborg.map")
        .joinpath("croatoan.resources")
        .read_text()
    )
