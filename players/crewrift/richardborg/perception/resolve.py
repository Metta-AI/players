...
def resolve_scene(scene: SceneState, tick: int) -> ResolvedScene:
    ...
    for object_id, obj in scene.objects.items():
        ...
        if CHAT_TEXT_OBJECT_BASE <= object_id < CHAT_TEXT_OBJECT_LIMIT:
            ...
            if label.startswith(PREFIX_PLAYER):
                ...
                candidate_cells.append(
                    (slot, label[len(PREFIX_BODY) :], False, obj.x, obj.y)
                )
            continue
        ...
        if PLAYER_OBJECT_BASE <= object_id < BODY_OBJECT_BASE and label.startswith(
            PREFIX_PLAYER
        ):
            ...
            BODY_OBJECT_BASE <= object_id < TASK_BUBBLE_OBJECT_BASE
            and label.startswith(PREFIX_BODY)
        ):
            bodies.append(
                VisibleBody(
                    object_id=object_id,
                    color=label[len(PREFIX_BODY) :],
                    world_x=coll_x,
                    world_y=coll_y,
                )
            )
        elif (
            TASK_BUBBLE_OBJECT_BASE <= object_id < TASK_ARROW_OBJECT_BASE
            and label == LABEL_TASK_BUBBLE
        ):
            tasks.append(
                TaskSignal(
            ...
            TASK_ARROW_OBJECT_BASE <= object_id < VOTE_DOT_OBJECT_BASE
            and label == LABEL_TASK_ARROW
        ):
            tasks.append(
                TaskSignal(
            ...
            VOTE_DOT_OBJECT_BASE
            <= object_id
            < VOTE_DOT_OBJECT_BASE + MAX_PLAYERS * MAX_PLAYERS
            ...
            VOTE_SKIP_DOT_OBJECT_BASE
            <= object_id
            < VOTE_SKIP_DOT_OBJECT_BASE + MAX_PLAYERS
            and label.startswith(PREFIX_VOTE_DOT)
        ):
            # Skip votes share the "vote dot" sprite but a separate id range.
            dots.append(
                VoteDot(
                    target=SKIP_VOTE_TARGET, voter=object_id - VOTE_SKIP_DOT_OBJECT_BASE
                )
            )
        elif (
    ...
    census = tuple(
        CensusEntry(color=c, alive=alive) for _slot, c, alive, _x, _y in candidate_cells
    )
    candidates = tuple(
        VoteCandidate(slot=s, color=c, alive=alive)
        for s, c, alive, _x, _y in candidate_cells
    )
...
def _cursor_slot(
    ...
    if cursor_xy is None or not cells:
        return None
    cx, cy = cursor_xy
    slot, _color, _alive, _x, _y = min(
        cells, key=lambda c: (c[3] - cx) ** 2 + (c[4] - cy) ** 2
    )
    return slot
