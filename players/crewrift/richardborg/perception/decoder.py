...
overlay (line of sight). Every other sprite's pixels are skipped — richardborg reads
...
from players.crewrift.richardborg.perception.tables import (
    LayerDef,
    ObjectState,
    SpriteDef,
)
...
def _decode_walkability(
    scene: SceneState, width: int, height: int, compressed: bytes
) -> None:
...
def _apply_define_layer(scene: SceneState, message: bytes, offset: int) -> int:
    ...
    scene.layers[layer_id] = LayerDef(
        layer_type=message[offset + 1], flags=message[offset + 2]
    )
    offset += 3
    return offset
