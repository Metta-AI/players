#!/usr/bin/env python3
"""Download full episode data for the N most recent episodes richardborg played.

Richardborg runs in the hosted Crewrift league; the Observatory data API records
...
2026-06-01):
  ...
  * logs/richardborg_slot{N}_v{V}.log -- richardborg's own per-tick stderr trace for
...
exists because it (1) filters straight to richardborg across all its policy
...
Usage (from anywhere; auth comes from `softmax login`):
    ...
    uv run python players/crewrift/richardborg/scripts/fetch_episodes.py -n 10
    uv run python .../fetch_episodes.py -n 5 --version 2 --out /tmp/richardborg_eps
...
pass --server to switch. The paths below are written for the gateway base.
  GET /stats/policy-versions?name_exact=richardborg              -> all richardborg policy versions
...
@dataclass
class EpisodeRecord:
    """One richardborg episode plus the handles needed to fetch its data."""
...
def discover_policy_versions(
    client: Client, version: int | None
) -> list[dict[str, Any]]:
    """All richardborg policy versions (id + version), newest first.

    Pass `version` to keep only that one. Auto-discovers future versions, so a
    newly uploaded richardborg:v3 needs no code change.
    """
    ...
    if version is not None:
        ...
        if not pvs:
            sys.exit(
                f"No richardborg policy version {version} found (have: "
...
def discover_episodes(
    client: Client, pvs: list[dict[str, Any]], want: int
) -> list[EpisodeRecord]:
        ...
        while True:
            try:
                rows = self._client.get_json(
                    "/v2/experience-request-episodes",
                    pool_id=pool_key,
                    limit=page,
                    offset=offset,
                )
...
def richardborg_slots(ereq: dict[str, Any]) -> list[dict[str, Any]]:
    """Participant entries in this ereq that are richardborg (one per slot it held)."""
    return [
        p
    ...
    *,
    ...
    if want_replay:
        if episode.replay_url:
            try:
                raw = httpx.get(
                    episode.replay_url, follow_redirects=True, timeout=120.0
                )
    ...
    # 4. richardborg's per-slot policy logs (needs the ereq for slot + pv).
    if want_logs and ereq is not None:
        slots = richardborg_slots(ereq)
        if slots:
            ...
            for p in slots:
                ...
                if text is None:
                    ...
                    continue
                name = f"richardborg_slot{slot}_v{ver}.log"
                (logs_dir / name).write_text(text)
                summary["logs"].append(name)
        else:
            summary["errors"].append("no richardborg participant in this episode")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Download full episode data for the N most recent richardborg episodes.",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Only this richardborg policy version (e.g. 2). Default: all.",
    )
    parser.add_argument(
        "--server",
        default=None,
    ...
    parser.add_argument(
        "--no-replay", action="store_true", help="Skip replay downloads."
    )
    parser.add_argument(
        "--no-logs", action="store_true", help="Skip richardborg policy-log downloads."
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    with Client(server, load_token()) as client:
        ...
        log(
            "richardborg policy versions: "
            + ", ".join(f"v{pv['version']}" for pv in pvs)
        )
        episodes = discover_episodes(client, pvs, args.num)
        if not episodes:
            log("No richardborg episodes found.")
            return 1
        ...
        for i, ep in enumerate(episodes, 1):
            ...
            stamp = (
                ep.created_at.replace(":", "").replace("-", "").replace(".", "")[:15]
            )
            ep_dir = args.out / f"{stamp}_{ep.episode_id[:8]}"
            if not args.force and episode_is_complete(ep_dir, want_replay, want_logs):
                log(
                    f"  [{i}/{len(episodes)}] {ep.episode_id[:8]} v{ep.version} — already present, skipping"
                )
                summaries.append(
                    {"episode_id": ep.episode_id, "dir": ep_dir.name, "skipped": True}
                )
                continue
            log(
                f"  [{i}/{len(episodes)}] {ep.episode_id[:8]} v{ep.version} {ep.created_at}"
            )
            s = fetch_episode(
                client,
                ereq_index,
