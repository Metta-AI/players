...
from players.crewrift.richardborg.nav import NavGraph, plan_route

if TYPE_CHECKING:
    from players.crewrift.richardborg.map.types import MapData
    from players.crewrift.richardborg.types import Belief, PerceptionFrame
...
class RoutePolyline(BaseModel):
    ...
    def point_at(self, arc_length: float) -> Point:
        ...
        return round(start[0] + (end[0] - start[0]) * frac), round(
    ...
    *,
    ...
    for start in anchors:
        for end in anchors:
            ...
            if route:
                polylines[(start.name, end.name)] = RoutePolyline.from_points(
                    [start.point, *route]
                )
...
def update_agent_tracking(belief: "Belief") -> None:
    ...
    frame = (
        ...
        if belief.recent_frames and belief.recent_frames[-1].tick == belief.last_tick
        else None
    )
    ...
    for color in visible_colors & live_crew_colors:
        ...
        if (
            ...
            and color not in tracking.previous_visible_colors
            and previous.top_point is not None
        ):
            actual_point = (
                frame.players[color]
                if frame is not None
                else (belief.roster[color].world_x, belief.roster[color].world_y)
            )
            tracking.reacquisitions.append(
                _reacquisition(
                    substrate, previous, belief.last_tick, color, actual_point
                )
            )

    estimates = _estimate_colors(
        substrate, belief, live_crew_colors, visible_colors, frame
    )
    teammate_estimates = _estimate_colors(
        substrate, belief, live_teammate_colors, visible_colors, frame
    )
    ...
    tracking.teammate_snapshot = (
        _snapshot(substrate, belief.last_tick, teammate_estimates.values())
        if teammate_estimates
        else None
    )
...
def _estimate_colors(
    ...
    for color in sorted(colors):
        ...
        if color in visible_colors and frame is not None:
            estimates[color] = _observed_estimate(
                substrate, color, belief.last_tick, frame.players[color]
            )
        else:
...
def ranked_seek_points(belief: "Belief") -> list[Point]:
    ...
    points: list[Point] = []
    ranked = sorted(
        snapshot.expected_by_cell.items(), key=lambda item: item[1], reverse=True
    )
    ...
    *,
    ...
    best = max(
        ...
        key=lambda target: (
            target.score,
            target.expected,
            -_dist2(self_xy, target.point),
            target.room_name,
        ),
    )
    if current_room_name is not None:
        current = next(
            (target for target in targets if target.room_name == current_room_name),
            None,
        )
        if (
            current is not None
            and current.score > 0
            and current.score >= best.score * ROOM_TARGET_HYSTERESIS
        ):
            return current
...
def _room_expected(
    substrate: OccupancySubstrate, expected_by_cell: dict[int, float]
) -> dict[str, float]:
    out: dict[str, float] = {}
...
def _room_center_cell(
    room_name: str, room_cells: dict[str, list[OccupancyCell]], map_data: "MapData"
) -> Point:
    room = next(
        (candidate for candidate in map_data.rooms if candidate.name == room_name), None
    )
...
def _anchors(nav: NavGraph, map_data: "MapData") -> list[TrackingAnchor]:
    ...
    out.append(
        TrackingAnchor(
            name="home",
            kind="home",
            point=_snap(nav, (map_data.home.x, map_data.home.y)),
        )
    )
    button_point = nav.button_anchor or (
        map_data.button.center.x,
        map_data.button.center.y,
    )
    out.append(
        TrackingAnchor(name="button", kind="button", point=_snap(nav, button_point))
    )
    for index, task in enumerate(map_data.tasks):
        point = nav.task_anchor(index) or (task.center.x, task.center.y)
        out.append(
            TrackingAnchor(
                name=f"task:{index}", kind="task", index=index, point=_snap(nav, point)
            )
        )
    return out
...
def _coarse_grid(
    nav: NavGraph, map_data: "MapData", cell_size: int
) -> tuple[dict[int, OccupancyCell], int, int]:
    ...
    for (row, col), points in by_cell.items():
        ...
        index = row * cols + col
        out[index] = OccupancyCell(
            index=index,
            row=row,
            col=col,
            center=center,
            label=_region_label(map_data, center),
        )
...
def _region_label(map_data: "MapData", point: Point) -> str | None:
    for room in map_data.rooms:
        if (
            room.x <= point[0] < room.x + room.w
            and room.y <= point[1] < room.y + room.h
        ):
            return room.name
    return None
...
def _reachability_estimate(
    ...
    *,
    ...
    support_radius = (
        radius + math.sqrt(2 * substrate.cell_size * substrate.cell_size) / 2
    )
    support = [
        cell
        for cell in substrate.cells.values()
        if math.dist(last_seen, cell.center) <= support_radius
        and not _point_visible(frame, cell.center)
    ]
    if not support:
        support = [
            cell
            for cell in substrate.cells.values()
            if not _point_visible(frame, cell.center)
        ]
    if not support:
    ...
    top = (
        min(support, key=lambda cell: (_dist2(last_seen, cell.center), cell.index))
        if support
        else None
    )
    return AgentPositionEstimate(
...
def _snapshot(
    ...
    top_expected = 0.0
    if expected:
        top_cell, top_expected = max(
            expected.items(), key=lambda item: (item[1], -item[0])
        )
...
def _reacquisition(
    ...
    distance_error = (
        math.dist(previous.top_point, actual_point)
        if previous.top_point is not None
        else None
    )
...
def _cell_for_point(
    substrate: OccupancySubstrate, point: Point
) -> OccupancyCell | None:
