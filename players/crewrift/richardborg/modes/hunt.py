...
from players.crewrift.richardborg.strategy.opportunity import select_victim, unwitnessed
from players.crewrift.richardborg.strategy.trajectory import lead_ticks, predict
...
class HuntMode(Mode[Belief, ActionState, Intent]):
    ...
    def __init__(self, params: ModeParams | None = None) -> None:
        ...
        self._victim_color: str | None = (
            None  # the crewmate we have committed to hunting
        )
    ...
    def decide(self, belief: Belief, action_state: ActionState) -> Intent:
        ...
        if victim is None:
            return Intent(
                kind="idle", reason="no victim to hunt"
            )  # selector normally flips to Search/Pretend
        ...
        if in_range and belief.self_kill_ready and unwitnessed(belief, victim):
            return Intent(
                ...
                reason="striking isolated victim",
            )
    ...
    def _resolve_victim(self, belief: Belief) -> PlayerRecord | None:
        ...
        current = (
            belief.roster.get(self._victim_color)
            if self._victim_color is not None
            else None
        )
        if (
            current is not None
