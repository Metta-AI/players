...
from players.crewrift.richardborg.agent_tracking import ranked_seek_points
...
from players.crewrift.richardborg.types import ActionState, Belief, Intent, PlayerRecord
...
class SearchMode(Mode[Belief, ActionState, Intent]):
    ...
    def decide(self, belief: Belief, action_state: ActionState) -> Intent:
        ...
        if target is not None:
            ...
            return Intent(
                ...
                reason="search: following visible target",
            )
        ...
        if self._target_color is not None:
            ...
            if (
                target is not None
                and target.life_status != "dead"
                and _trackable(belief, target)
            ):
                ...
                if ic.dist2(self_xy, target_xy) > ARRIVE_RADIUS_SQ:
                    return Intent(
                        kind="navigate_to",
                        point=target_xy,
                        reason="search: following last-seen target",
                    )
    ...
    def _target(self, belief: Belief) -> PlayerRecord | None:
        current = (
            belief.roster.get(self._target_color)
            if self._target_color is not None
            else None
        )
        if (
    ...
    def _search_occupancy(self, belief: Belief, self_xy: ic.Point) -> Intent:
        if (
            self._search_point is None
            or ic.dist2(self_xy, self._search_point) <= ARRIVE_RADIUS_SQ
        ):
        ...
        return Intent(
            kind="navigate_to",
            point=self._search_point,
            reason="searching likely crew occupancy",
        )
    ...
    def _next_search_point(self, belief: Belief, self_xy: ic.Point) -> ic.Point | None:
        ...
        for point in points:
            if ic.dist2(self_xy, point) <= ARRIVE_RADIUS_SQ:
                continue
            if any(
                ic.dist2(point, visited) <= ARRIVE_RADIUS_SQ
                for visited in self._visited_points
            ):
                continue
