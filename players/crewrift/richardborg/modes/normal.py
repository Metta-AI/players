...
from players.crewrift.richardborg.map.types import TaskStation
...
class NormalMode(Mode[Belief, ActionState, Intent]):
    ...
    def decide(self, belief: Belief, action_state: ActionState) -> Intent:
        ...
        if self._target is not None:
            return Intent(
                ...
                reason="completing assigned task",
            )
    ...
    def _update_target(self, belief: Belief, tasks: tuple[TaskStation, ...]) -> None:
        ...
        if target is not None and target < len(tasks):
            on_station = _inside(
                tasks[target], belief.self_world_x, belief.self_world_y
            )
            if on_station and belief.active_task_progress_pct is not None:
                self._max_progress = max(
                    self._max_progress, belief.active_task_progress_pct
                )
    ...
    def _pick_target(
        self, belief: Belief, tasks: tuple[TaskStation, ...], signals: set[int]
    ) -> int | None:
        ...
        return min(
            candidates, key=lambda i: _dist2(self_xy, _nav_point(belief, tasks[i], i))
        )

    def _sweep_intent(
        self, belief: Belief, tasks: tuple[TaskStation, ...]
    ) -> Intent | None:
        ...
        if (
            belief.assigned_task_indices
            or not tasks
            or belief.crew_tasks_remaining == 0
        ):
            return None
        self_xy = _self_xy(belief)
        if self_xy is None:
        ...
        nearest = min(
            remaining, key=lambda i: _dist2(self_xy, _nav_point(belief, tasks[i], i))
        )
        return Intent(
            kind="navigate_to",
            point=_nav_point(belief, tasks[nearest], nearest),
...
def _return_to_start(belief: Belief) -> Intent:
    ...
    return Intent(
        kind="navigate_to", point=goal, reason="tasks done: returning to the start room"
    )
