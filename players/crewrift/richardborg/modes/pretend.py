...
from players.crewrift.richardborg.agent_tracking import best_pretend_room_target
from players.crewrift.richardborg.modes import imposter_common as ic
from players.crewrift.richardborg.map.types import Room
from players.crewrift.richardborg.types import ActionState, Belief, Intent
...
class PretendMode(Mode[Belief, ActionState, Intent]):
    ...
    def _goto_room(self, belief: Belief, self_xy: ic.Point) -> Intent:
        ...
        if (
            ...
            and self._goto_point is not None
            and ic.dist2(self_xy, self._goto_point) <= ARRIVE_RADIUS_SQ
        ):
            station = self._task_station or _station_in_room(
                belief, target_room, self_xy
            )
            if station is not None and ic.dist2(self_xy, station) <= ARRIVE_RADIUS_SQ:
                self._state, self._task_station, self._hold_until = (
                    "do_task",
                    station,
                    None,
                )
        ...
        if (
            self._goto_point is None
            or ic.dist2(self_xy, self._goto_point) <= ARRIVE_RADIUS_SQ
        ):
            self._target_room_name = None
        ...
        reason = (
            "pretending at likely crew task"
            if self._room_chosen_tick is not None
            else "pretending at fallback task"
        )
    ...
    def _do_task(self, belief: Belief, self_xy: ic.Point) -> Intent:
        ...
        if ic.dist2(self_xy, self._task_station) > ARRIVE_RADIUS_SQ:
            return Intent(
                kind="navigate_to",
                point=self._task_station,
                reason="heading to a task station",
            )
    ...
    def _room_target_committed(self, belief: Belief, self_xy: ic.Point) -> bool:
        if (
            self._target_room_name is None
            or self._goto_point is None
            or self._room_chosen_tick is None
        ):
            return False
...
def _station_in_room(belief: Belief, room: Room, self_xy: ic.Point) -> ic.Point | None:
    ...
    indices = [
        ...
        if ic.in_rect((tasks[i].center.x, tasks[i].center.y), room)
    ]
    if not indices:
        return None
    nearest = min(
        indices, key=lambda i: ic.dist2(self_xy, (tasks[i].center.x, tasks[i].center.y))
    )
