# Richardborg — Agent Notes

**Richardborg** is a Player-SDK–based agent that plays the Coworld game
**Crewrift**. This file is the orientation doc: what the relevant codebases are,
how they fit together, the API/protocol richardborg must satisfy, and pointers to
...
> LLM chat/vote path with deterministic fallback. `RICHARDBORG_BE_DUMB=1` is an
> aggressive imposter experiment that skips Pretend/Evade/body reports and keeps
> the imposter in Search/Hunt. `RICHARDBORG_DICK_MODE=1` is an opt-in crewmate
...
> [`design.md`](./design.md) for the settled architecture. richardborg sits at
> `players/players/crewrift/richardborg/` inside the `players` uv workspace.

---

## The big picture

Three layers. Richardborg is the top; it treats the protocol below as a contract.

```
  richardborg (this dir)              the agent we are building
  ...
  Player SDK  (players.player_sdk) generic two-loop "Cyborg" agent framework
        │ binary Sprite-v1 WebSocket  (richardborg writes this bridge itself)
        ▼
  Crewrift game server (Nim)       authoritative game; defines the rules
```

- **Crewrift** is an _Among Us–style social-deduction game_ (crewmates do tasks
  ...
  coordinates** plus sprites that carry **text labels**, _specifically so agents
  read game state from structured data and labels — no computer vision_
...
- **The Player SDK** (`players.player_sdk`) is a generic Python framework for
  building agents with a _fast symbolic inner loop_ + a _slower strategy loop_
  (the architecture is historically called "Cyborg" in the SDK docs). It is
  transport- and game-agnostic.
- **Richardborg** plugs Crewrift-specific perception/belief/modes/strategy into the
  SDK's runtime, and ships as a Docker image the Coworld runner launches.

**Key consequence:** the SDK gives you the _loop and the scaffolding_
(`AgentRuntime`, `Mode`, `ModeDirective`, strategy runners, tracing). You supply
_everything game-specific_: maintain Crewrift's scene state (objects + their
labels and coordinates) as belief, decide modes, and resolve intents into
Sprite-v1 button/text packets. Richardborg also owns its **own websocket bridge** —
...
2. **`perceive(observation, tick) -> percept`** — interpret the scene: join each
   ...
   Keep raw sprite pixels _out_ of belief.
...
4. **`resolve_action(intent, belief, action_state) -> command`** — translate a
   symbolic intent into wire packets. _All_ transport mechanics live here
...
### Transport: the part the SDK does _not_ do for you
...
That is for _grid/token_ games (cogsguard-style), **not** for a pixel game.
...
So richardborg must **write its own websocket bridge** that:
...
intermediate frames) that richardborg does not currently need, since it has no rate
...
Richardborg's contract. Two checkouts matter:
...
- **Coworld platform docs:** `~/coding/metta/packages/coworld` _(inside the
  metta checkout — **read-only**, never write there)_ — the generic Coworld
...
scene data, and Crewrift uses it specifically so agents do _not_ need computer
...
| **Objects** | `u16` object id | **x, y** (`i16`; _camera-relative_ on `/player` — see below), z (draw order), layer, **sprite id** |
...
(`0x05`) of **128×128**, then Define-sprite (`0x01`) for all _static_ sprites
...
important implementation fact and the protocol doc does _not_ spell it out for
...
−mapObject.x`, `worldX = obj.x + cameraX` — exactly what `notsus` does at
  `np:496-499`.
- **Your own player is _not_ an object** — it's the implicit camera center. Your
  ...
  `SpriteDrawOffY=8` at `sim:38-40`). Read your own _color/role/state_ from HUD
...
- World/HUD: `map` (`global:1736`), `walkability map` (`global:1744`),
  ...
  `global:1784`), `imposter icon` / `imposter icon cooldown` (⇒ _you_ are
  imposter, kill ready / cooling, `global:1760,1768`), `ghost icon` (⇒ _you_ are
...
the `map` sprite's pixels (`buildMapSpritePixels`, `global:701`). richardborg can't
...
nav graph over it (§6). The only other sprite whose pixels richardborg decodes is the
...
(`0x82`/`0x83`) is _not_ read by Crewrift's logic — don't rely on it. Verified
...
So **richardborg's perception is structured-scene maintenance**, not vision: keep
...
pixel-CV stack — richardborg's perception is **structured-scene maintenance**, not
...
**Step cadence (a richardborg design choice, grounded in the server).** The server
...
and sends input _only if the mask changed_. `notsus`' `receiveLatestFrameInto`
(`np:611`) also drains any already-queued messages to act on the freshest frame —
a latency optimization richardborg skips (it has no rate limiter and self-corrects
...
Richardborg ships as a Linux/amd64 Docker image; **stdout = protocol channel,
stderr = logs/traces**. Upload/submit with the `coworld` CLI:

```sh
docker buildx build --platform linux/amd64 -t richardborg:latest --load .
coworld upload-policy richardborg:latest --name richardborg
coworld submit richardborg:v1 --league <crewrift-league-id>
```
...
`slots.items.properties.name`; (2) pass `--run /srv/players/players/crewrift/richardborg/coworld/entrypoint.sh`,
...
authority). _(The flat `COWORLD_README.md`/`GAME_RUNTIME_README.md` were
reorganized into `docs/` as of coworld 0.1.13.)_

**Retrieving hosted episodes richardborg played.** The Observatory API records
every league episode. **Use [`scripts/fetch_episodes.py`](./scripts/fetch_episodes.py)**
for a richardborg-filtered bulk pull of replays + per-slot traces + metadata: it
...
`strategy/meeting/` and is enabled by `RICHARDBORG_LLM_MEETINGS=1` plus
`ANTHROPIC_API_KEY`, or by Coworld Bedrock policy env (`USE_BEDROCK=true`, set by
`upload-policy --use-bedrock`). `RICHARDBORG_LLM_MEETINGS=1` plus
...
_validate_ a baked map — vent/button/task locations are not in the stream) and the
...
- **Crewrift's `notsus` / `evidencebot_v2` Nim bots** are the behavior references;
  `notsus`' Sprite-v1 _parsing_ code (`players/notsus/notsus/protocols.nim`) — how
...
- **`players/cogsguard/{baseline,buggy,cranky}`** — JSON/`coworld_json_bridge`
  players (token-grid game). **Not** richardborg's transport, but a good
...
uv sync
uv run pytest players/crewrift/richardborg/tests      # richardborg tests
uv run ruff check players/crewrift/richardborg
```
...
| Richardborg design decisions                     | `players/crewrift/richardborg/design.md`                                                                                                                                                          |
...
| Coworld platform/runner contract                 | `~/coding/metta/packages/coworld/src/coworld/docs/README.md` + `runner/runner.py` _(read-only)_                                                                                                   |
| Fetch hosted episodes richardborg played         | `players/crewrift/richardborg/scripts/fetch_episodes.py` (the typed `coworld episodes`/`replays`/`episode-logs` are 404-broken since the server's `episode-requests`→`experience-request` rename) |
| View richardborg trace replays                   | `players/crewrift/richardborg/viewer/index.html` (load logs captured with `RICHARDBORG_TRACE=viewer` or `RICHARDBORG_TRACE=debug`)                                                                |

Absolute roots:

- Player SDK & this workspace: `~/coding/players_checkouts/players` (pkg `players`)
- Crewrift game source: `~/coding/games/coworld-crewrift`
- Coworld platform: `~/coding/metta/packages/coworld` _(read-only — metta checkout)_
...
- `notsus` is a **dual-path** bot: a legacy pixel-CV path _and_ the label path.
