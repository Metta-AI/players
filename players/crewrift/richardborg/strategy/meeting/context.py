...
from players.crewrift.richardborg.perception.entities import SKIP_VOTE_TARGET
from players.crewrift.richardborg.strategy.meeting.schema import (
    CHAT_MAX_CHARS,
    SCHEMA_VERSION,
    VOTE_SKIP,
)
...
def _fallback_vote_reason(belief: Belief, fallback_vote: str) -> str:
    ...
    return (
        f"top suspect {fallback_vote} at P(imposter)={p:.4f}"
        if p is not None
        else "top suspect"
    )
...
def _chat_ready(belief: Belief, last_chat_tick: int | None) -> bool:
    return (
        last_chat_tick is None
        or belief.last_tick - last_chat_tick >= CHAT_COOLDOWN_TICKS
    )
...
def _voting_payload(belief: Belief) -> dict[str, Any]:
    ...
    for dot in voting.dots:
        target = (
            ...
            else slot_to_color.get(dot.target, str(dot.target))
        )
        tally[target] += 1
        dots.append(
            {
        ...
        "messages": [
            {
                ...
                "self": event.speaker_color == self_color
                or event.text in sent_chat_texts,
                "text": event.text,
            }
...
def _player_payload(belief: Belief, color: str, record: PlayerRecord) -> dict[str, Any]:
    age = (
        ...
        else max(0, belief.last_tick - record.last_seen_tick)
    )
    return {
        "color": color,
        ...
        "last_seen_xy": [record.world_x, record.world_y]
        if record.last_seen_tick
        else None,
        ...
        "recent_events": [
            _event_payload(belief, event) for event in record.events[-8:]
        ],
    }
    ...
    return {
        ...
        "ranking": [
            ...
            for color, p in sorted(
                belief.suspicion.items(), key=lambda item: item[1], reverse=True
            )
        ],
