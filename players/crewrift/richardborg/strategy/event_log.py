...
def update_event_log(belief: Belief) -> None:
    ...
    visible = [
        ...
        if r.last_seen_tick == tick and r.life_status != "dead"
    ]
    if not visible:
        return
    ...
    for record in visible:
        ...
        for body in belief.bodies.values():
            ...
            if d2 <= NEAR_BODY_RADIUS_SQ:
                _mark(
                    record, tick, prev, "near_body", target_color=body.color, dist2=d2
                )
        ...
        for other in visible:
            ...
            if d2 <= KILL_RANGE_SQ:
                _mark(
                    record, tick, prev, "proximity", target_color=other.color, dist2=d2
                )
...
def _mark(
    ...
    *,
    ...
    if (
        latest is not None
        and latest.end_tick == prev
        and tick - latest.end_tick <= EVENT_MERGE_GRACE_TICKS
    ):
        latest.end_tick = tick
        if dist is not None:
            latest.min_dist = (
                dist if latest.min_dist is None else min(latest.min_dist, dist)
            )
        return
...
def _latest_matching(
    ...
    for event in reversed(record.events):
        if (
            event.kind == kind
            and event.region_index == region_index
            and event.target_color == target_color
        ):
            return event
    return None
