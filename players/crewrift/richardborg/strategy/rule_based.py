...
2. with ``RICHARDBORG_DICK_MODE=1``, once the first kill cooldown is near ready →
...
Crewmate nuisance experiment: ``RICHARDBORG_DICK_MODE=1`` (or ``DICK_MODE=1``)
...
class RuleBasedStrategy:
    ...
    def _select(self, belief: Belief, action_state: ActionState) -> ModeDirective:
        ...
        if phase == "Voting":
            ...
            return ModeDirective(
                mode="attend_meeting", source="strategy", reason="meeting open"
            )
        ...
        if phase == "Playing":
            ...
            if belief.self_role == "dead":
                ...
                return ModeDirective(
                    mode="normal", source="strategy", reason="ghost: finish own tasks"
                )
...
def _be_dumb_enabled() -> bool:
    return _truthy_env("RICHARDBORG_BE_DUMB") or _truthy_env("BE_DUMB")


def _dick_mode_enabled() -> bool:
    return _truthy_env("RICHARDBORG_DICK_MODE") or _truthy_env("DICK_MODE")
