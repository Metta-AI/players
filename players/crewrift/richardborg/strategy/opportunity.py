...
from players.crewrift.richardborg.nav import plan_route
...
def select_victim(belief: Belief) -> PlayerRecord | None:
    ...
    if belief.nav is not None:
        candidates = [
            t for t in crew if plan_route(belief.nav, self_xy, (t.world_x, t.world_y))
        ]
        if not candidates:
            return None
    unclaimed = [
        ...
        if not _claimed_by_teammate(target, belief, self_xy)
    ]
    if unclaimed:
    ...
    return max(
        candidates,
        key=lambda t: (_isolation(t, belief), -_dist2(self_xy, (t.world_x, t.world_y))),
    )
...
def _isolation(target: PlayerRecord, belief: Belief) -> float:
    ...
    gaps = [
        ...
        if o.color != target.color
        and o.color not in belief.teammate_colors
        and o.life_status != "dead"
    ]
    return min(gaps) if gaps else float("inf")


def _is_unwitnessed(
    target: PlayerRecord, belief: Belief, radius_sq: float, window: int
) -> bool:
...
def _claimed_by_teammate(
    target: PlayerRecord, belief: Belief, self_xy: tuple[int, int]
) -> bool:
    target_xy = (target.world_x, target.world_y)
