...
2026-06-01):
  ...
  * episode_request.json  -- the matching `/v2/episode-requests/{ereq}` row:
                             every participant (name + slot + policy version),
                             status, scores, and the concrete game_config.
...
episode in a single pass, and (3) reads raw JSON, so it is immune to coworld
client/server model skew (the workspace was on coworld 0.1.11, whose
`V2EpisodeRequestRow` required an `assignments` field the server had dropped,
crashing the typed CLI commands with a `ValidationError`; fixed by upgrading to
>= 0.1.13). The per-ereq `artifacts/{type}` endpoint is a dead end (rejects
results/replay/config with "Unknown artifact type"); the replay comes from the
episode's `replay_url` and the traces from the per-agent `policy-logs` endpoint
-- both confirmed working. See the module-level NOTES for the endpoint map.
...
NOTES -- live endpoint map. Base defaults to the official gateway
`<softmax.auth.get_api_server()>/observatory` (today: https://softmax.com/api/observatory),
the same route the `coworld` CLI uses. The Observatory service is also reachable
directly at https://api.observatory.softmax-research.net, but there the routes
sit at the host *root* (no `/observatory` segment); pass --server for either.
  ...
  GET /v2/episode-requests?pool_id=pool_<uuid>&limit     -> ereq rows for a pool (to map episode->ereq)
  GET /v2/episode-requests/{ereq}/{pv}/policy-logs/{slot} -> a participant's stderr trace
        ...
        while True:
            try:
                rows = self._client.get_json(
                    "/v2/episode-requests", pool_id=pool_key, limit=page, offset=offset
                )
        ...
        if slots:
            ...
            for p in slots:
                ...
                text = client.get_text_or_none(
                    f"/v2/episode-requests/{ereq['id']}/{pv}/policy-logs/{slot}"
                )
                if text is None:
