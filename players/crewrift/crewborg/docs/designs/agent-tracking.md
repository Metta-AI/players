...
## 5. Prerequisite (offline bake): anchors + A\* polylines + the grid

Generated into `map/croatoan_prebaked.npz` (extends main design §6), all from the
existing `NavGraph`/`plan_route`:
...
- Offline bake: anchors + polylines + grid. Per tick: ≤7 agents × ~10 hypotheses ×
...
1. **Static substrate + tracing.** Anchors, pairwise A* polylines, coarse grid, and a
   ...
   `agent_tracking.py` + `events.py`; generated by `scripts/export_prebaked_map.py`.**
   Check: polylines & grid sane in traces; no first-tick bake cost.
