import
  ...
  suspectra/[protocols, votereader]
...
import std/[algorithm, exitprocs, heapqueue, json, monotimes, options, os,
  osproc, parseopt, random, strutils, times]
...
const
  ...
  LlmHelperDefault = "/srv/suspectra/llm_meeting.py"
...
type
  ...
  Bot = ref object
    ...
    voteConfirmTicks: int
    llmMeetingStartTick: int
    llmDecisionReady: bool
    llmVoteTarget: int
    llmInstantVote: bool
    llmChatText: string
    llmReason: string
    llmLastAttemptTick: int
...
proc setSelfColor(bot: var Bot, colorIndex: int)
...
proc clearVotingState(bot: var Bot) =
  ...
  bot.llmMeetingStartTick = -1
  bot.llmDecisionReady = false
  bot.llmVoteTarget = VoteUnknown
  bot.llmInstantVote = false
  bot.llmChatText = ""
  bot.llmReason = ""

proc votingCarryState(bot: Bot): VotingCarryState =
  ## Captures meeting state that survives one vote-frame parser refresh.
  VotingCarryState(
    voteDelayTicks: bot.voteDelayTicks,
    voteQueuedSusColor: bot.voteQueuedSusColor,
    voteImposterChatDecided: bot.voteImposterChatDecided,
    voteLoggedTarget: bot.voteLoggedTarget,
    voteLoggedReason: bot.voteLoggedReason,
    voteConfirmTarget: bot.voteConfirmTarget,
    voteConfirmTicks: bot.voteConfirmTicks,
    llmMeetingStartTick: bot.llmMeetingStartTick,
    llmDecisionReady: bot.llmDecisionReady,
    llmVoteTarget: bot.llmVoteTarget,
    llmReason: bot.llmReason,
    llmInstantVote: bot.llmInstantVote,
    llmChatText: bot.llmChatText
  )

proc restoreVotingCarryState(bot: var Bot, state: VotingCarryState) =
  ## Restores meeting state after fresh vote-screen observations are loaded.
  bot.voteDelayTicks =
    if state.voteDelayTicks >= 0:
      state.voteDelayTicks
    else:
      bot.randomVoteDelay()
  bot.voteQueuedSusColor = state.voteQueuedSusColor
  bot.voteImposterChatDecided = state.voteImposterChatDecided
  bot.voteLoggedTarget = state.voteLoggedTarget
  bot.voteLoggedReason = state.voteLoggedReason
  bot.voteConfirmTarget = state.voteConfirmTarget
  bot.voteConfirmTicks = state.voteConfirmTicks
  bot.llmMeetingStartTick = state.llmMeetingStartTick
  bot.llmDecisionReady = state.llmDecisionReady
  bot.llmVoteTarget = state.llmVoteTarget
  bot.llmReason = state.llmReason
  bot.llmInstantVote = state.llmInstantVote
  bot.llmChatText = state.llmChatText
...
proc resetRoundState(bot: var Bot) =
  ...
  bot.forcedSelfColorIndex = -1
  bot.llmLastAttemptTick = -1
  bot.lastVoteFrame = ""
...
proc protocolVoteCursorCellAt(count, x, y: int): int =
  ## Maps protocol vote cursor origins without overlapping adjacent cells.
  if count <= 0:
    return VoteUnknown
  let
  ...
  for i in 0 ..< count:
    ...
    if x >= cellX and x < cellX + VoteCellW and
...
proc applyProtocolVotingState(
  ...
  let
    startTick =
      if bot.voting and bot.voteStartTick >= 0:
        ...
        bot.frameTick
    carryState = bot.votingCarryState()
  bot.clearVotingState()
  bot.voting = true
  bot.votePlayerCount = playerCount
  bot.voteStartTick = startTick
  bot.restoreVotingCarryState(carryState)
  ...
  if selfSlot >= 0 and selfSlot < playerCount:
    bot.setSelfColor(slots[selfSlot].colorIndex)
    ...
    of "SKIP":
      ...
      if actor.found:
        if protocolSelfObject(
          ...
          bot.setSelfColor(actor.colorIndex)
...
proc setSelfColor(bot: var Bot, colorIndex: int) =
  ## Updates self color while respecting the Coworld slot identity.
  if bot.forcedSelfColorIndex >= 0:
    bot.selfColorIndex = bot.forcedSelfColorIndex
  elif colorIndex >= 0 and colorIndex < PlayerColorCount:
...
proc parseVotingCandidate(
  ...
  let carryState = bot.votingCarryState()
  bot.clearVotingState()
  bot.voting = true
  ...
  for i in 0 ..< count:
    ...
    if bot.voteSelfMarkerPresent(count, i, slots[i].colorIndex):
      bot.voteSelfSlot = i
      bot.setSelfColor(slots[i].colorIndex)
...
proc parseVotingScreen(bot: var Bot): bool {.measure.} =
  ...
  if read.found:
    ...
    if read.selfSlot >= 0 and read.selfSlot < read.playerCount:
      bot.setSelfColor(read.slots[read.selfSlot].colorIndex)
    bot.voteChatLines.setLen(0)
...
proc voteTargetName(bot: Bot, target: int): string =
  ...
  if target == VoteSkip or (
      bot.votePlayerCount > 0 and target == bot.votePlayerCount
  ):
    return "skip"
...
proc envFlagEnabled(name: string): bool =
  ## Returns true when an environment flag is explicitly enabled.
  let value = getEnv(name).normalizeChatText()
  value in ["1", "true", "yes", "on"]

proc llmMeetingsEnabled(): bool =
  ## Returns true when meeting LLM calls should be attempted.
  envFlagEnabled("SUSPECTRA_LLM_MEETINGS") or
    envFlagEnabled("USE_BEDROCK") or
    envFlagEnabled("CLAUDE_CODE_USE_BEDROCK")

proc voteTargetFromLlmName(bot: Bot, name: string): int =
  ## Maps an LLM vote target name onto one currently selectable vote slot.
  ...
  for target in 0 ..< bot.votePlayerCount:
    if bot.voteTargetSafeForRole(target) and
        bot.voteTargetName(target).normalizeChatText() == normalized:
      return target
  VoteUnknown

proc canonicalMeetingObservations(bot: Bot): JsonNode =
  ...
  if bot.bodySusColor != VoteUnknown:
    result.add(%("I saw " & playerColorName(bot.bodySusColor) &
      " near the reported body."))
  for line in bot.voteChatLines:
    let accusedColor = chatSusColorIndex(line.text)
    if accusedColor != VoteUnknown and line.speakerColor != VoteUnknown:
      result.add(%(line.voteChatSpeakerName() & " accused " &
        playerColorName(accusedColor) & "."))
  if result.len == 0:
    result.add(%"No hard visual evidence has been recorded this meeting.")

proc meetingLlmContext(
  bot: Bot,
  fallbackTarget: int,
  fallbackReason: string,
  listenedTicks: int
): JsonNode =
  ## Serializes notsus's parsed meeting state for the LLM helper.
  result = newJObject()
  result["schema_version"] = %1
  result["trigger"] = %"meeting_vote"
  result["self"] = %*{
    "color": playerColorName(bot.selfColorIndex),
    "role": roleName(bot.role)
  }
  result["meeting"] = %*{
    "start_tick": bot.voteStartTick,
    "listened_ticks": listenedTicks,
    "vote_timer_ticks": VoteDeadlineTicks
  }
  let fallbackName =
    if fallbackTarget == bot.votePlayerCount:
      "skip"
    else:
      bot.voteTargetName(fallbackTarget)
  result["state"] = %*{
    "fallback_vote": fallbackName,
    "fallback_vote_reason": fallbackReason,
    "current_vote_target": bot.voteTargetName(bot.voteTarget)
  }

  var validTargets = newJArray()
  for target in 0 ..< bot.votePlayerCount:
    if bot.voteTargetSafeForRole(target):
      validTargets.add(%bot.voteTargetName(target))
  validTargets.add(%"skip")
  result["constraints"] = %*{
    "actions": ["send_chat", "set_tentative_vote", "submit_vote", "wait"],
    "valid_vote_targets": validTargets,
    "skip_vote_target": "skip",
    "chat_max_chars": VoteChatChars,
  ...
  for target in 0 ..< bot.votePlayerCount:
    players.add(%*{
      "slot": target,
      "color": playerColorName(bot.voteSlots[target].colorIndex),
      "alive": bot.voteSlots[target].alive,
      "self": target == bot.voteSelfSlot
    })
  result["players"] = players

  var votes = newJArray()
  for colorIndex, choice in bot.voteChoices:
    if choice == VoteUnknown:
      continue
    votes.add(%*{
      "voter_color": playerColorName(colorIndex),
      "target": bot.voteTargetName(choice)
    })
  result["votes"] = votes
  ...
  for line in bot.voteChatLines:
    chat.add(%*{
      "speaker_color": line.voteChatSpeakerName(),
      "text": line.text
    })
  result["chat"] = chat

  result["memory"] = %*{
    "summary_md": "Use direct observations only. Weak sightings are not proof. Vote only on body, vent, kill, contradiction, or coordinated vote evidence.",
    "templates_md": [
      "I saw {player} and {other} together.",
      "I saw {player} with {victim} shortly before {victim} died.",
      "I saw {player} near {victim}'s body.",
      "I saw {player} at a vent.",
      "I directly confirmed {player} as an imposter from a kill or vent transition."
    ],
    "canonical_observations": bot.canonicalMeetingObservations()
  }

proc maybeApplyMeetingLlm(
  bot: var Bot,
  fallbackTarget: int,
  fallbackReason: string,
  listenedTicks: int
) =
  ## Lets one bounded helper call override meeting chat/vote, else keeps notsus.
  if bot.llmDecisionReady:
    return
  if bot.llmLastAttemptTick >= 0 and
      bot.frameTick - bot.llmLastAttemptTick < LlmRetrySuppressTicks:
    return
  ...
  if VoteDeadlineTicks - listenedTicks < LlmMinRemainingTicks:
    return
  if not llmMeetingsEnabled():
    return

  bot.llmLastAttemptTick = bot.frameTick
  bot.llmMeetingStartTick = bot.voteStartTick
  bot.llmDecisionReady = true

  let
    timeoutExe = findExe("timeout")
    pythonExe = findExe(getEnv("SUSPECTRA_PYTHON", "python3"))
  if timeoutExe.len == 0 or pythonExe.len == 0:
    echo "meeting llm disabled: timeout/python3 command missing"
    return

  let
    helper = getEnv("SUSPECTRA_LLM_HELPER", LlmHelperDefault)
    seconds = getEnv(
      "SUSPECTRA_LLM_PROCESS_TIMEOUT_SECONDS",
      LlmProcessTimeoutDefault
    )
    command = timeoutExe.quoteShell() & " " & seconds.quoteShell() & " " &
      pythonExe.quoteShell() & " " & helper.quoteShell()
    context = $bot.meetingLlmContext(fallbackTarget, fallbackReason, listenedTicks)
    run = execCmdEx(command, options = {poUsePath}, input = context)

  if run.exitCode != 0:
    echo "meeting llm fallback: helper exit ", run.exitCode
    return
  let output = run.output.strip()
  if output.len == 0:
    echo "meeting llm fallback: helper returned no output"
    return

  let response = output.parseJson()
  let
    action = response["action"].getStr()
    chatText = response["chat_text"].getStr().strip()
    voteName = response["vote_target"].getStr()
    reason = response["reason"].getStr()
    target = bot.voteTargetFromLlmName(voteName)

  if chatText.len > 0 and bot.pendingChat.len == 0:
...
proc decideVotingMask(bot: var Bot): uint8 {.measure.} =
  ...
  bot.maybeApplyMeetingLlm(decision.target, decision.reason, listenedTicks)
  var
    voteReason = decision.reason
    instantVote = decision.instant
  if bot.llmVoteTarget != VoteUnknown:
  ...
  if bot.voteCursor != bot.voteTarget:
    ...
    return bot.desiredMask
  let waitTicks =
    if bot.role == RoleImposter and not instantVote:
      VoteImposterSkipTicks
    else:
      bot.voteDelayTicks
  if not instantVote and listenedTicks < waitTicks:
    bot.desiredMask = 0
    bot.controllerMask = 0
  ...
  if bot.voteConfirmTarget == bot.voteTarget:
    inc bot.voteConfirmTicks
  else:
    bot.voteConfirmTarget = bot.voteTarget
    bot.voteConfirmTicks = 1
  if bot.voteConfirmTicks < VoteCursorConfirmTicks:
    bot.desiredMask = 0
    bot.controllerMask = 0
    bot.intent = "confirming vote cursor on " & bot.voteTargetName(bot.voteTarget)
    bot.thought(bot.intent)
    return 0
  bot.desiredMask =
    if bot.lastMask == ButtonA:
...
proc initBot(mapPath = ""): Bot {.measure.} =
  ...
  result.forcedSelfColorIndex = -1
  result.llmLastAttemptTick = -1
...
when not defined(italkalotLibrary):
  proc runBot(
    ...
    if slot >= 0 and slot < PlayerColorCount:
      bot.forcedSelfColorIndex = slot
      bot.selfColorIndex = slot
      echo "forcing self color from slot ", slot, " (", playerColorName(slot), ")"
    let endpoint =
...
when isMainModule and not defined(italkalotLibrary):
  ...
  proc readBotRunConfig(): BotRunConfig =
    ...
    if not urlSet and (addressSet or portSet):
      result.url = ""
    if result.slot < 0 and result.url.len > 0:
      let marker = "slot="
      let start = result.url.find(marker)
      if start >= 0:
        var finish = start + marker.len
        while finish < result.url.len and result.url[finish] in {'0'..'9'}:
          inc finish
        if finish > start + marker.len:
          result.slot = parseInt(result.url[start + marker.len ..< finish])
