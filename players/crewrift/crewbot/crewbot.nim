...
const
  ...
  LlmHelperDefault = "/srv/crewbot/llm_meeting.py"
...
proc llmMeetingsEnabled(): bool =
  ...
  envFlagEnabled("CREWBOT_LLM_MEETINGS") or
...
proc maybeApplyMeetingLlm(
  ...
  let
    ...
    pythonExe = findExe(getEnv("CREWBOT_PYTHON", "python3"))
  if timeoutExe.len == 0 or pythonExe.len == 0:
    echo "meeting llm disabled: timeout/python3 command missing"
    return

  let
    helper = getEnv("CREWBOT_LLM_HELPER", LlmHelperDefault)
    seconds = getEnv(
      "CREWBOT_LLM_PROCESS_TIMEOUT_SECONDS",
