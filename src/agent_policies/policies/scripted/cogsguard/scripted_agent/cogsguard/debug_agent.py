...
from agent_policies.policies.scripted.cogsguard.scripted_agent.cogsguard.types import StructureType
...
class DebugHarness:
    ...
    def from_recipe(
        ...
        from mettagrid.policy.prepare_policy_spec import prefer_installed_package_code  # noqa: PLC0415
        ...
        params = signature(make_env).parameters
        env_kwargs: dict[str, Any] = {}
        if "max_steps" in params:
            env_kwargs["max_steps"] = max_steps
        if "num_agents" in params:
            env_kwargs["num_agents"] = num_agents
        elif "cogs_per_team" in params and "num_teams" in params:
            num_teams_default = params["num_teams"].default
            num_teams = int(num_teams_default) if isinstance(num_teams_default, int) else 4
            if num_agents % num_teams != 0:
                raise ValueError(
                    f"Recipe make_env requires cogs_per_team/num_teams, but num_agents={num_agents} "
                    f"is not divisible by num_teams={num_teams}"
                )
            env_kwargs["num_teams"] = num_teams
            env_kwargs["cogs_per_team"] = num_agents // num_teams
        env_cfg = make_env(**env_kwargs)
