#!/usr/bin/env python3
"""Scout training v6: pure PPO with clean scout_gained=10.0 reward only.

Key insight from debugging: random agents get scout gear by step 3 (it's trivially
close to spawn). The v1-v5 failures were likely caused by confounding reward signals:
- cell_visited (0.00001) may cause entropy collapse / deterministic walk
- non-scout role penalties (-1.0 each) confuse credit assignment since
  role stations are clustered near c:scout

v6 approach: ONLY reward scout_gained. Let PPO discover the +10.0 signal naturally.
"""
...
class ScoutOnlyReward(CoGameMissionVariant):
    """Only reward: scout_gained=10.0. No cell_visited, no role penalties."""
    name: str = "scout_only_reward"
    description: str = "Clean scout_gained=10.0 reward, nothing else."
...
mission = CvCMission(
    name="scout_v6",
    description="Scout v6: clean scout_gained reward only.",
...
print("Reward: scout_gained=10.0 ONLY")
...
train_config = dict(
    ...
    vf_clip_coef=0.2,
    max_grad_norm=1.5,
    ent_coef=0.03,  # higher entropy to prevent collapse
    vtrace_rho_clip=1.0,
    vtrace_c_clip=1.0,
    prio_alpha=0.8,
    prio_beta0=0.2,
...
print(f"\nStarting v6 training: {TOTAL_TIMESTEPS:,} steps")
