#!/usr/bin/env python3
"""Scout training with kickstarting via post-hoc reward shaping.

After each evaluate() call, compares stored student actions with what
the scripted teacher would have done. Adds a reward bonus for matching.
The bonus is annealed to zero over the first half of training.

NO PufferLib modifications required — only modifies trainer.rewards
between evaluate() and train() calls.
"""
...
from scout_teacher import get_teacher_action
...
# Kickstarting reward shaping parameters
KS_BONUS = 0.05        # Per-step reward bonus when matching teacher
KS_ANNEAL_FRAC = 0.5   # Anneal bonus to 0 over first 50% of training

mission = CvCMission(
    name="scout_kickstart",
    description="Scout kickstarting: scripted teacher + reward shaping.",
...
print(f"Kickstarting: bonus={KS_BONUS}, anneal_frac={KS_ANNEAL_FRAC}")
...
print(f"\nStarting kickstarted training: {TOTAL_TIMESTEPS:,} steps")
...
while trainer.global_step < train_config["total_timesteps"]:
    ...
    # Post-hoc reward shaping: bonus for matching scripted teacher
    progress = trainer.global_step / train_config["total_timesteps"]
    bonus = KS_BONUS * max(0.0, 1.0 - progress / KS_ANNEAL_FRAC)

    if bonus > 0:
        # Access stored observations and actions
        obs_buf = trainer.observations  # [segments, horizon, 200, 3]
        act_buf = trainer.actions       # [segments, horizon]

        # Move to CPU for teacher computation
        obs_np = obs_buf.cpu().numpy()
        act_np = act_buf.cpu().numpy().astype(np.int32)

        segs, horizon = obs_np.shape[0], obs_np.shape[1]
        matches = 0
        total = 0

        for s in range(segs):
            for h in range(horizon):
                teacher_act = get_teacher_action(obs_np[s, h])
                if act_np[s, h] == teacher_act:
                    trainer.rewards[s, h] += bonus
                    matches += 1
                total += 1
    ...
    if trainer.global_step % (BATCH_SIZE * 10) == 0:
        ...
        if ks_total > 0:
            match_rate = ks_matches / ks_total
            eff_bonus = KS_BONUS * max(0.0, 1.0 - progress / KS_ANNEAL_FRAC)
            print(f"  [KS] match_rate={match_rate:.3f}, bonus={eff_bonus:.4f}, progress={progress:.2%}")
            ks_matches = 0
            ks_total = 0
