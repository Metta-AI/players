#!/usr/bin/env python3
"""Evaluate scout checkpoint v2: larger eval + action distribution check.

Usage: python eval_scout_v2.py [checkpoint_path]
"""
...
def check_scout_gear(obs, agent_idx):
    ...
    for t in range(o.shape[0]):
        coord, feat, val = int(o[t, 0]), int(o[t, 1]), int(o[t, 2])
...
def main():
    ...
    if checkpoint_path:
        ...
        policy.eval()
        print("Loaded:", checkpoint_path, "on", device)
    else:
        print("No checkpoint - random policy")

    print("Episodes:", NUM_EPISODES, "Agents:", NUM_AGENTS)
    print("=" * 60)
    ...
    for ep in range(NUM_EPISODES):
        ...
        sim.add_event_handler(StatsTracker(NoopStatsWriter()))
        sim.add_event_handler(EarlyResetHandler())
        env = MettaGridPufferEnv(sim, cfg.model_copy(deep=True), buf=None, seed=ep * 7 + 13)
        obs, info = env.reset()
        ...
        all_scout.append(sum(scout_gained))
        all_reward.append(total_reward.mean())
        all_steps.append(steps)
        if ep < 5 or sum(scout_gained) > 0:
            print("Ep {:2d}: steps={:4d} scout={} reward_mean={:.2f}".format(
                ep, steps, scout_gained, total_reward.mean()))
        env.close()

    print("\n" + "=" * 60)
    print("SUMMARY ({} episodes)".format(NUM_EPISODES))
    print("=" * 60)
    sg = np.array(all_scout)
    print("Total scout gear: {} / {} episodes ({} per ep)".format(
        sg.sum(), NUM_EPISODES, sg.sum() / NUM_EPISODES))
    print("Episodes with scout: {} / {}".format((sg > 0).sum(), NUM_EPISODES))
    print("Mean reward: {:.2f}".format(np.mean(all_reward)))
    print("Mean steps: {:.1f}".format(np.mean(all_steps)))
    print()
    total_act = sum(all_actions.values())
    print("Action distribution:")
    for i in range(5):
        pct = all_actions[i] / total_act * 100 if total_act > 0 else 0
        print("  {}: {:6.2f}% ({})".format(ACTION_NAMES[i], pct, all_actions[i]))
