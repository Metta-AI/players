...
init_workspace() {
  ...
  log "Workspace initialized at $REPO_ROOT"
}
...
save_snapshot() {
  ...
  mkdir -p "$dest"
  cp -r "$REPO_ROOT/$POLICY_PATH/" "$dest/robot/"
...
### Your task:
1. Read the current robot policy code in $POLICY_PATH/
2. If present, read competing policy snapshots under policies/ for transferable ideas
...
7. If the score regressed, revert with: cd $REPO_ROOT && git checkout -- $POLICY_PATH
...
main_loop() {
  ...
  BASELINE_SCORE=$(python "$WORKDIR/eval_harness.py" \
    --seeds "$FULL_SEEDS" \
    --policy "$POLICY_SPEC" 2>&1 | tail -1 || echo "0")
  ...
  while true; do
    ...
    python "$WORKDIR/eval_harness.py" \
      --seeds "$FULL_SEEDS" \
      --policy "$POLICY_SPEC" 2>&1 | tee -a "$LOG_FILE" || {
