...
The robot policy lives in `policies/robot/`. Key files:
...
Softy is in `policies/Softy:v77/softy.py` (1721 lines). Study it. Key advantages:
...
Dinky Bob uses compiled Nim bindings (`policies/dinky_bob:v5/dinky/`). It's a NimMultiAgentPolicy with pre-compiled native code. You can't read the logic directly, but it uses scramblers (30-177 per episode based on tournament data) and has sophisticated pathfinding. Treat Softy as the primary reference since you can read its full source.
...
1. **Only edit files in `policies/robot/`.** Never modify game rules, cogames, or other policies.
...
3. **Git checkpoint improvements.** After any score improvement >= 2 points:
   ```bash
   cd /app/policies && git add -A robot/ && git commit -m "robot: <description> (avg: X.X -> Y.Y)"
   ```
...
5. **Save policy snapshots.** Copy policy dir at milestones:
   ```bash
   cp -r /app/policies/robot /app/snapshots/robot-v<N>-score-<X>/
   ```
...
8. **Study Softy.** When stuck, re-read `policies/Softy:v77/softy.py` for proven patterns.
...
4. **Implement the change** in the relevant `policies/robot/*.py` files
