...
# Sync the policies directory
log "Syncing policies..."
rsync -avz --progress \
  --exclude '__pycache__' \
  --exclude '*.pyc' \
  --exclude '.git' \
  --exclude 'dinky_bob:v5/dinky/.git' \
  "$PROJECT_ROOT/policies/" \
  "$CLUSTER:~/cvc-optimizer/policies/"

# Sync game docs for reference
log "Syncing docs..."
rsync -avz --progress \
  "$PROJECT_ROOT/docs/" \
  "$CLUSTER:~/cvc-optimizer/docs/"
...
# Set up env vars
export POLICIES_DIR=~/cvc-optimizer/policies
export AWS_DIR=~/.aws
