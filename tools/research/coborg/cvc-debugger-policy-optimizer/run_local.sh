...
export POLICIES_DIR
export RESULTS_FILE
export PYTHONPATH="${POLICIES_DIR}:${PYTHONPATH:-}"
...
init() {
  ...
  log "Policies: $POLICIES_DIR"
  log "Results: $RESULTS_FILE"
}

run_eval() {
  local seeds="${1:-$FULL_SEEDS}"
  python3 "$SCRIPT_DIR/eval_harness.py" --seeds "$seeds" --policy "class=robot.RobotPolicy" --workers 4 2>&1
}
...
### Your task:
1. Read the current robot policy code in $POLICIES_DIR/robot/
2. Read the competing Softy policy in $POLICIES_DIR/Softy:v77/softy.py for ideas
...
7. If the score regressed, revert with: cd $POLICIES_DIR && git checkout -- robot/
