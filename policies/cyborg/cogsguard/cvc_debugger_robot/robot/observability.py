...
Activation (inline with game):
  ROBOT_DEBUG=1 cogames play -m arena -p class=policies.cyborg.cogsguard.cvc_debugger_robot.robot.RobotPolicy
...
def _build_app(hub: ObservabilityHub):
  ...
  from policies.cyborg.cogsguard.cvc_debugger_robot.robot.game_runner import GameRunner
  ...
  async def api_anomaly_toggle(request: Request):
    ...
    if hub._anomaly_detector is None:
      from policies.cyborg.cogsguard.cvc_debugger_robot.robot.anomaly_detector import AnomalyDetector
  ...
  async def api_investigate(request: Request):
    ...
    from policies.cyborg.cogsguard.cvc_debugger_robot.robot.investigator import Investigator
  ...
  from policies.cyborg.cogsguard.cvc_debugger_robot.robot.eval_engine import EvalEngine
  ...
  async def api_eval_stream(request: Request):
    ...
    from policies.cyborg.cogsguard.cvc_debugger_robot.robot.eval_engine import POLICIES_DIR, _parse_json_score, _parse_log_score
    body = await request.json()
    seed = body.get("seed", 42)
    steps = body.get("steps", 2000)
    mission = body.get("mission", "machina_1")
  ...
  async def api_agent_chat(request: Request):
    ...
    import asyncio
    from policies.cyborg.cogsguard.cvc_debugger_robot.robot.policy_agent import PolicyAgent
    loop = asyncio.get_event_loop()
