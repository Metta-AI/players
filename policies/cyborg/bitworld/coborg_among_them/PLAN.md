...
1. Lives in `~/coding/agent-policies/` at
   `policies/cyborg/bitworld/coborg_among_them/`
...
- `policies/cyborg/bitworld/among_them/__init__.py` —
...
4. Trace events appear on stderr in the container log; stdout shows only
   protocol traffic.
5. `pytest policies/cyborg/bitworld/coborg_among_them/tests`
   is green.
...
- `coworld/entrypoint.sh`: forwards args to
  `python -m policies.cyborg.bitworld.coborg_among_them.coworld.policy_player "$@"`.
...
| Existing scripted Among Them (state-vector reference) | `~/coding/agent-policies/policies/cyborg/bitworld/among_them/__init__.py` |
...
| This plan | `~/coding/agent-policies/policies/cyborg/bitworld/coborg_among_them/PLAN.md` |
