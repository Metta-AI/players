...
PolicySpec for CogonyPolicy. The policy's `record_dir` kwarg writes
...
from cogony_policy.viewer.render import render as render_report
...
# CogonyPolicy kwargs we pass through to PolicySpec.init_kwargs. Validated
...
def _validate_policy_kwargs(policy_kwargs: dict[str, Any]) -> None:
    ...
    if bad:
        raise ValueError(
            f"unknown CogonyPolicy kwarg(s): {sorted(bad)}. "
...
def _drive_rollout(*, env_cfg: Any, spec: PolicySpec, run_dir: Path, seed: int) -> int:  # pragma: no cover
    ...
    `CogonyPolicy._on_episode_end` synchronously after rollout (to flush
...
def run_scenario(
    ...
    *,
    ...
    spec = PolicySpec(
        class_path="cogony_policy.cogamer_policy.CogonyPolicy",
