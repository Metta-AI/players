...
Architecture:
  CogonyPolicy (MultiAgentPolicy)
    └─ StatefulAgentPolicy[CogonyAgentState]  (one per agent)
...
from cogony_policy.llm_worker import LLMWorker, WORLD_MODEL_ATTR_SKIP
from cogony_policy.programs import Program, all_programs
...
_TRACE_DIR = os.environ.get("COGONY_TRACE_DIR", "/tmp/cogony-trace")
...
class CogonyPolicyImpl(StatefulPolicyImpl[CogonyAgentState]):
    ...
    def initial_agent_state(self) -> CogonyAgentState:
        ...
        state = CogonyAgentState(game_state=gs)
    ...
    def step_with_state(self, obs: AgentObservation, state: CogonyAgentState) -> tuple[Action, CogonyAgentState]:
        gs = state.game_state
...
class CogonyPolicy(MultiAgentPolicy):
    ...
    short_names = ["cogony", "cogony-policy"]
    ...
    def agent_policy(self, agent_id: int) -> StatefulAgentPolicy[CogonyAgentState]:
    ...
    def reset(self) -> None:
        ...
        # constructs fresh CogonyPolicyImpl + initial_agent_state (new GameState,
    ...
    def _stop_workers(self) -> None:
        for wrapper in self._agent_policies.values():
            st: CogonyAgentState | None = getattr(wrapper, "_state", None)
